...
$.fn.popup = function(options) {
    ...
    if( opts.type == 'browser' ) {
        ...
        popup =  $("<div id='h_yacu"+ opts.name + "' class='"+ opts.name + "'></div>").css({
            'display' : 'none',
            'position': 'absolute' ,
            'z-index' : '1000' ,
            'width'   :  opts.width,
        ...
        var popIframe = $("<iframe src='" + opts.url +  "' width='100%' height='100%' marginwidth='0' marginheight='0' frameborder='0' scrolling='"+ opts.scroll +"' style='border-width:1px; border-color:#B2B2B2; border-style:solid;'></iframe>");
        popup.append(popIframe);
        
        if($("body").has("#h_yacu"+ opts.name).html() != null){
        	$("#h_yacu"+ opts.name).remove();
        	$("body").prepend(popup);
		}else{
			$("body").prepend(popup);
		}
