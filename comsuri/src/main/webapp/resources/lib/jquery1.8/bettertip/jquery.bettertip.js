...
var BT_open_wait = 0; //time in millis to wait before showing dialog
...
function BT_hoverIn(a)
{
	...
	var title = jQuery("#"+a.id).attr("titleNm");
	
	if(!BT_titles[a.id])
	{
		if(!title || title.toLowerCase() == "J$none")
			title = "";
		else if(title.toLowerCase() == "J$content")
			title = jQuery(a).text();
...
function BT_show(id) {
    ...
    if(typeof query["height"] == "undefined" ){
		...
		tipHeight = "height:"+query["height"]+"px;";
	}
	...
	if(jQuery("#BT").html() == null){
		
		jQuery("body").append(
			"<div id='BT' class='BT_shadow0' style='top:"+(top-shadowTop-8)+"px; left:"+(left-shadowLeft - 8)+"px;z-index:10'>" +
