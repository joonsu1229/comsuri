...
Handsontable.Core = function (rootElement, userSettings) {
  ...
  this.isArea = false;	// jung 추가, 문서 상에 스프레드 시트 바깥쪽에서 이벤트가 발생 했을 경우 에디터에 포커스가 가지 않도록 처리 하기 위해 추가
  ...
  datamap = {
    ...
    /**
     * 병합된 셀 중에 숨겨져 있는 셀들에 mergeCell 정보를 업데이트 하는 함수이다.
     * @author JUNG(jgs135@naonsoft.com)
     * @param {Array} [mergeCells] required. mergeCells은 병합된 셀들 중에 화면에 표시되는 셀들에 row,col 정보를 담고 있다.
     * @param {boolean} [isRow] required. 열 생성인지 여부 
     */
    updateMergeCell : function(mergeCells, isRow){
    	if(mergeCells.length > 0){
	    	for(var i=0; i < mergeCells.length; i++){
	    		var row = mergeCells[i].row;
	    		var col = mergeCells[i].col;        		
	    		var cellProperties = priv.cellSettings[row][col];
	    		var rowMax = cellProperties.rowSpan ? parseInt(cellProperties.rowSpan)+row : row+1;
	    		var colMax = cellProperties.colSpan ? parseInt(cellProperties.colSpan)+col : col+1;
	    		
	    		for(var j=row; j < rowMax; j++){
	    			for(var k=col; k < colMax; k++){
	    				if(row == j && col == k){
	    					continue;
	    				}
	    				var hiddenCellProperties = priv.cellSettings[j][k];
	    				
	    				if(hiddenCellProperties.data){
	    					if(isRow){
		    					hiddenCellProperties.data.row = hiddenCellProperties.data.row + 1;
		    				}else{
		    					hiddenCellProperties.data.col = hiddenCellProperties.data.col + 1;
		    				}
	    				}
	    			}
	    		}
      ...
      while (numberOfCreatedRows < amount && instance.countRows() < priv.settings.maxRows) {
        ...
        if (index === instance.countRows()) {
          ...
          GridSettings.prototype.data.splice(index, 0, row);
	       // jung 추가, 행 추가 시 중간에 병합된 셀이 있을 경우 화면에 td도 같이 이동 할 수 있도록 수정
	          var mergeCells = [];
	      	priv.cellSettings.splice(currentIndex,0, []);
	      	
	      	if(priv.cellSettings[currentIndex]){
	      		for(var i=0; i < instance.countVisibleCols(); i++){
		      		priv.cellSettings[currentIndex].push(new priv.columnSettings[i]());
		      		var cellProperties = instance.getCellMeta(currentIndex, i);
		      		var nextCellProperties = instance.getCellMeta(currentIndex+1, i);
		      		
		      		//기존에 있던 셀에 테두리 정보를 가져와 새로 만들어지는 셀에 넣어 준다.
		          	cellProperties.style.borderLeft = nextCellProperties.style.borderLeft;
		          	cellProperties.style.borderRight = nextCellProperties.style.borderRight;
		          	cellProperties.style.borderTop = nextCellProperties.style.borderTop;
		          	cellProperties.style.borderBottom = nextCellProperties.style.borderBottom;
		      		
		      		//아래 셀이 병합된 셀일 경우 추가되는 셀도 같이 병합되도록 처리 하는 부분
		      		if(nextCellProperties.style.display == 'none'){
		      			// 단일 열에서의 병합의 경우 아래 셀에 display 정보에 따라 표시 할지 여부를 결정 하면 되지만
		          		// 여러열이 병합된 경우에는 병합된 셀에 첫번째 셀이 추가되는 행보다 앞쪽인지를 확인 하여 표시 여부를 결정 해야만 한다.
		      			if(nextCellProperties.data.row < currentIndex){
		              		cellProperties.style = {};
		              		cellProperties.style.display = 'none';
		              		cellProperties.data = {
		              				row : nextCellProperties.data.row,
		              				col : nextCellProperties.data.col
		              		};
		              		
		              		if(nextCellProperties.col == nextCellProperties.data.col){
		              			var parentCellProperties = instance.getCellMeta(nextCellProperties.data.row,nextCellProperties.data.col);        		
		                  		parentCellProperties.rowSpan = parentCellProperties.rowSpan ? parseInt(parentCellProperties.rowSpan)+1 : 2;
		                  		//console.log(parentCellProperties.rowSpan);
		              		}
		      			}
		      		}
		      		
		      		// cellSettings 배열에 추가된 열에 의해 cellProperties가 한칸 씩 밀리게 된다. 이때 밀린 cellProperties들의
		          	// row 정보를 업데이트 해줘야 한다.
		          	// 또한 추가된 열 뒤에 병합된 셀들이 있다면 병합된 셀들에 mergeCell 정보도 변경해 줘야 한다.
		          	// mergeCell 값을 변경 하기 위해 병합된 셀에 첫번째 셀에 row, col 정보를 배열에 담고 아래에서
		          	// 숨겨져 있는 셀들에 mergeCell 정보를 변경 하게 된다.
		          	for(var j=currentIndex; j < instance.countRows(); j++){
		          		var cellProperties = instance.getCellMeta(j,i);
		          		
		          		if(j != currentIndex){
		          			cellProperties.row = cellProperties.row+1;
		          		}
		          		
		          		if(cellProperties.colSpan > 1 || cellProperties.rowSpan > 1){
		          			mergeCells.push({row : j, col : i});
		          		}
		          	}
		      	}
		      	
		      	this.updateMergeCell(mergeCells, true);
		      	// jung 추가 end
	      	}
        }
    ...
    createCol: function (index, amount) {
		...
		while (numberOfCreatedCols < amount && instance.countCols() < priv.settings.maxCols){
		 ...
		 // jung 추가, 열 추가 시 중간에 병합된 셀이 있을 경우 화면에 td도 같이 이동 할 수 있도록 수정
	        var mergeCells = [];
	        for(var i=0; i < instance.countVisibleRows(); i++){
	        	priv.cellSettings[i].splice(currentIndex,0, new priv.columnSettings[currentIndex]());
	        	var cellProperties = instance.getCellMeta(i,currentIndex);
	        	var siblingCellProperties = instance.getCellMeta(i,currentIndex+1);
	        	
	        	//기존에 있던 셀에 테두리 정보를 가져와 새로 만들어지는 셀에 넣어 준다.
	        	cellProperties.style.borderLeft = siblingCellProperties.style.borderLeft;
	        	cellProperties.style.borderRight = siblingCellProperties.style.borderRight;
	        	cellProperties.style.borderTop = siblingCellProperties.style.borderTop;
	        	cellProperties.style.borderBottom = siblingCellProperties.style.borderBottom;
	      	  
	        	//오른쪽 셀이 병합된 셀일 경우 추가되는 셀도 같이 병합되도록 처리 하는 부분
	        	if(siblingCellProperties.style.display == 'none'){
	        		
	        		// 단일 행에서의 병합의 경우 다음 셀에 display 정보에 따라 표시 할지 여부를 결정 하면 되지만
	        		// 여러행이 병합된 경우에는 병합된 셀에 첫번째 셀이 추가되는 열보다 앞쪽인지를 확인 하여 표시 여부를 결정 해야만 한다.
	        		if(siblingCellProperties.data.col < currentIndex){
	            		cellProperties.style.display = 'none';
	            		cellProperties.data = {
	            				row : siblingCellProperties.data.row,
	            				col : siblingCellProperties.data.col
	            		};
	            	
	            		if(siblingCellProperties.row == siblingCellProperties.data.row){
	            			var parentCellProperties = instance.getCellMeta(siblingCellProperties.data.row,siblingCellProperties.data.col);        		
	                		parentCellProperties.colSpan = parentCellProperties.colSpan ? parseInt(parentCellProperties.colSpan)+1 : 2;
	            		}
	        		}
	        	}
	        	
	        	// cellSettings 배열에 추가된 열에 의해 cellProperties가 한칸 씩 밀리게 된다. 이때 밀린 cellProperties들의
	        	// col, prop 정보를 업데이트 해줘야 한다.
	        	// 또한 추가된 열 뒤에 병합된 셀들이 있다면 병합된 셀들에 mergeCell 정보도 변경해 줘야 한다.
	        	// mergeCell 값을 변경 하기 위해 병합된 셀에 첫번째 셀에 row, col 정보를 배열에 담고 아래에서
	        	// 숨겨져 있는 셀들에 mergeCell 정보를 변경 하게 된다.
	        	for(var j=currentIndex; j < instance.countCols(); j++){
	        		var cellProperties = instance.getCellMeta(i,j);
	        		
	        		if(j != currentIndex){
	        			cellProperties.col = cellProperties.col+1;
	        			cellProperties.prop = cellProperties.prop+1;
	        		}
	        		
	        		if(cellProperties.colSpan > 1 || cellProperties.rowSpan > 1){
	        			mergeCells.push({row : i, col : j});
	        		}
	        	}
	        }
	        
	        this.updateMergeCell(mergeCells, false);
	        // jung 추가 end
		  }
    ...
    removeRow: function (index, amount) {
      ...
      if(amount == instance.countRows()){
    	  naon.ui.alert({message : '폼릿에 모든 셀을 삭제할 수 없습니다.'});
    	  return false;
      }
      ...
      instance.forceFullRender = true; //used when data was changed
      // jung 추가, 삭제 할 행에 병합된 셀이 포함 되어 있을 때 병합된 셀이 줄어 들거나 삭제 되지 않는 오류를 막기 위해 추가
      for(var a = 0; a < amount; a++){
    	  for(var i=0; i < instance.countCols(); i++){
        	  var cellProperties = instance.getCellMeta(index+a, i);
        	  
        	  // 숨겨져 있는 셀의 경우 부모 셀에 rowSpan 값을 -1 해준다.
        	  if(cellProperties.style.display == 'none'){
        		  if(cellProperties.col == cellProperties.data.col){
        			  var parentCellProperties = instance.getCellMeta(cellProperties.data.row, cellProperties.data.col);
        			  parentCellProperties.rowSpan = parseInt(parentCellProperties.rowSpan) - 1;
        			  priv.cellSettings[cellProperties.data.row][cellProperties.data.col] = parentCellProperties;
        		  }
        		   
        	  }
        	  
        	  // 병합된 셀에 모체가 삭제행에 있을 때 병합 정보를 아래행에 있는 셀에 넘긴다. 그리하여 병합된 셀이 삭제 되도
        	  // 병합을 유지하게 된다.
        	  if(cellProperties.rowSpan > 1){
        		  var nextCellProperties = instance.getCellMeta(index+a+1, i);
        		  nextCellProperties = cellProperties;
        		  nextCellProperties.row = nextCellProperties.row + 1;
        		  nextCellProperties.rowSpan = parseInt(nextCellProperties.rowSpan) -1;
        		  priv.cellSettings[index+a+1][i] = nextCellProperties;
        	  }
          }
      }
      
      // 숨겨져 있는 셀들의 mergeCell.row 정보를 업데이트 한다.
      for(var i=index; i < instance.countVisibleRows(); i++){
    	  for(var j=0; j < instance.countVisibleCols(); j++){
    		  var cellProperties = instance.getCellMeta(i,j);
    		  
    		  if(cellProperties.style.display == 'none'){
    			  if(cellProperties.data.row > index){
    				  cellProperties.data.row = cellProperties.data.row - amount;
    			  }
    		  }
    	  }
      }
    ...
    removeCol: function (index, amount) {
      ...
      for(var a = 0; a < amount; a++){
    	  for(var i=0; i < instance.countRows(); i++){
        	  var cellProperties = instance.getCellMeta(i, index+a);
        	  
        	  // 숨겨져 있는 셀의 경우 부모 셀에 rowSpan 값을 -1 해준다.
        	  if(cellProperties.style.display == 'none'){
        		  if(cellProperties.row == cellProperties.data.row){
        			  var parentCellProperties = instance.getCellMeta(cellProperties.data.row, cellProperties.data.col);
        			  parentCellProperties.colSpan = parseInt(parentCellProperties.colSpan) - 1;
        			  priv.cellSettings[cellProperties.data.row][cellProperties.data.col] = parentCellProperties;
        		  }
        	  ...
        	  // 병합된 셀에 모체가 삭제행에 있을 때 병합 정보를 아래행에 있는 셀에 넘긴다. 그리하여 병합된 셀이 삭제 되도
        	  // 병합을 유지하게 된다.
        	  if(cellProperties.colSpan > 1){
        		  var nextCellProperties = instance.getCellMeta(i, index+a+1);
        		  nextCellProperties = cellProperties;
        		  nextCellProperties.col = nextCellProperties.col + 1;
        		  nextCellProperties.prop = nextCellProperties.prop + 1;
        		  nextCellProperties.colSpan = parseInt(nextCellProperties.colSpan) -1;
        		  priv.cellSettings[i][index+a+1] = nextCellProperties;
        	  }
          }
      }
      ...
      // 숨겨져 있는 셀들의 mergeCell.row 정보를 업데이트 한다.
      for(var i=0; i < instance.countVisibleRows(); i++){
    	  for(var j=index; j < instance.countVisibleCols(); j++){
    		  var cellProperties = instance.getCellMeta(i, j);
    		  
    		  if(cellProperties.style.display == 'none'){
    			  if(cellProperties.data.col > index){
    				  cellProperties.data.col = cellProperties.data.col - amount;
    			  }
    		  }
    	  }
      }
    ...
    /**
     * 셀 속성 정보를 리턴하는 함수 ( 2차 배열)
     * @author JUNG(jgs135@naonsoft.com)
     * @param {Object} start (Optional) Start selection position
     * @param {Object} end (Optional) End selection position
     * @return {Array}
     */
    getCellProperties: function (start, end) {
    	var properties = [];
    	for(var i=start.row; i <= end.row; i++){
    		var rowProperties = []; 
    		for(var j=start.col; j <= end.col; j++){
    			var obj = instance.copyObject(instance.getCellMeta(i, j));
    			
    			// 열/행 전체 복사 시 병합된 셀이 있을 경우 병합된 셀들 전체가 선택 영역에 포함 되는지 확인 하여 체크 하는 부분이다.
    			if(obj.data){
    				if(obj.data.row < start.row || obj.data.row > end.row || obj.data.col < start.col || obj.data.col > end.col){
    					obj.isNotCopy = true;
    				}
    			}
    			
    			if(obj.colSpan > 1 || obj.rowSpan > 1){
    				var row = parseInt(obj.row)+parseInt(obj.rowSpan)-1, col = parseInt(obj.col)+parseInt(obj.colSpan)-1;
    				if(obj.row < start.row || row > end.row || obj.col < start.col || col > end.col){
    					obj.isNotCopy = true;
    				}
    			}
    			
    			rowProperties.push(obj);
    		}
    		properties.push(rowProperties);
    	}
        return properties;
    }
  };
  ...
  grid = {
    ...
    alter: function (action, index, amount, source, keepEmptyRows) {
      ...
      switch (action) {
        case "insert_row":
          ...
          if (delta) {
        	  // 행 추가 시 리사이징된 높이 값을 저장 하는 manualRowHeights 배열에
        	  // 추가된 행 만큼 값을 추가한다.
        	 if(instance.manualRowHeights.length > 0){
        		 var array1 = instance.manualRowHeights.slice(0,index);
        		 var array2 = instance.manualRowHeights.slice(index);
        		 
        		 for(var i=0; i < amount; i++){
        			 array1.push(undefined);
        		 }     
        		 instance.manualRowHeights = array1.concat(array2);
             }
            ...
            if (priv.selStart.exists() && priv.selStart.row() >= index) {
              ...
              selection.transformStart(-delta, 0); //will call render() internally
              
              // 행 추가 시 첫번째 셀이 병합된 셀일 경우 선택 영역을 잘 못 잡는 오류가 있어서 아래 내용을 삽입
              // 첫번째 셀에 display가 none인 경우는 행 전체를 선택 했을 때만 발생하는 경우이다.
              var firstCell = instance.getCellMeta(index,0);
              if(firstCell.style.display == 'none'){
            	  /* transformStart를 하게 되면 셀이 병합되어 있어서 화면에 표시 되지 않을 경우 
            	   * 화면에 표시되는 첫번째 셀로 선택영역이 이동하게 된다. 이를 보정 하기 위해
            	   * 첫번째 셀이 display가 none이면 시작 index값으로 선택영역을 이동 시킨다. 
            	   * 
            	   */
            	  var coords = {
            			  row : index,
              ...
              // 선택 영역의 열 개수가 변경되는 걸 보정 해준다
              if(selected[1] != selected[3]){
            	  var maxColSize = Math.max(selected[1], selected[3]);
            	  var minColSize = Math.min(selected[1], selected[3]);
            	  
            	  if(minColSize != 0) --maxColSize;
            	  
            	  selection.transformEnd(amount-1,maxColSize);
              }
            }
            else {
        ...
        case "insert_col":
          ...
          if (delta) {
        	  // 열 추가 시 리사이징된 너비 값을 저장 하는 manualColumnWidths 배열에
        	  // 추가된 열 만큼 값을 추가한다.
        	  if(instance.manualColumnWidths.length > 0){
         		 var array1 = instance.manualColumnWidths.slice(0,index);
         		 var array2 = instance.manualColumnWidths.slice(index);
         		 
         		 for(var i=0; i < amount; i++){
         			 array1.push(undefined);
         		 }        		 
         		 instance.manualColumnWidths = array1.concat(array2);
              }
        	  
            if (priv.selStart.exists() && priv.selStart.col() >= index) {
              priv.selStart.col(priv.selStart.col() + delta);
              var selected = instance.getSelected();
              selection.transformStart(0, -delta); //will call render() internally
              
              // 열 추가 시 첫번째 셀이 병합된 셀일 경우 선택 영역을 잘 못 잡는 오류가 있어서 아래 내용을 삽입
              // 첫번째 셀에 display가 none인 경우는 열 전체를 선택 했을 때만 발생하는 경우이다.
              var firstCell = instance.getCellMeta(0,index);
              if(firstCell.style.display == 'none'){
            	  /* transformStart를 하게 되면 셀이 병합되어 있어서 화면에 표시 되지 않을 경우 
            	   * 화면에 표시되는 첫번째 셀로 선택영역이 이동하게 된다. 이를 보정 하기 위해
            	   * 첫번째 셀이 display가 none이면 시작 index값으로 선택영역을 이동 시킨다. 
              ...
              // 선택 영역의 행 개수가 변경되는 걸 보정 해준다
              if(selected[0] != selected[2]){
            	  var maxRowSize = Math.max(selected[0], selected[2]);
            	  var minRowSize = Math.min(selected[0], selected[2]);
            	  
            	  if(minRowSize != 0) --maxRowSize;            	  
            	  selection.transformEnd(maxRowSize,amount-1);
              }
            }
            else {
              selection.refreshBorders(); //it will call render and prepare methods
        ...
        case "remove_row":
          ...
          // 행 삭제 시 리사이징된 높이 값을 저장 하는 manualRowHeights 배열에
    	  // 삭제된 행 만큼 배열에서 제거해 준다.
          if(instance.manualRowHeights.length > 0){
        	  instance.manualRowHeights.splice(index, amount);
          }
        ...
        case "remove_col":
          ...
          // 열 삭제 시 리사이징된 너비 값을 저장 하는 manualColumnWidths 배열에
    	  // 삭제된 열 만큼 배열에서 제거해 준다.
          if(instance.manualColumnWidths.length > 0){
        	  instance.manualColumnWidths.splice(index, amount);
          }
     ...
     */
    populateFromArray: function (start, input, end, source, method, cellProperties) {
      var r, rlen, c, clen, setData = [], setCellData = [], current = {};
      rlen = input.length;
      if (rlen === 0) {
        return false;
      }
      ...
      switch (method) {
        ...
        default:
          ...
          for (r = 0; r < rlen; r++) {
        	  // jung 추가, 붙여 넣기 시에 행이 모자를 경우 행을 추가한다.
        	if(!priv.settings.minSpareRows && start.row+r > instance.countRows() - 1){
        		instance.alter('insert_row',null,1);
        	}  
        	
            if ((end && current.row > end.row) || (current.row >= priv.settings.maxRows)) {
            	break;
            }
            current.col = start.col;
            clen = input[r] ? input[r].length : 0;
            for (c = 0; c < clen; c++) {
              // jung추가, 분여 넣기 시에 열이 모자를 경우 열을 추가한다.	
              if(!priv.settings.minSpareCols && start.col+c > instance.countCols() - 1){
            	  instance.alter('insert_col',null,1);
              }	
              ...
              if (!instance.getCellMeta(current.row, current.col).readOnly) {
                // jung 수정, 붙여 넣기 할 때 셀복사있지 텍스트 복사인지 구분하여 복사 하도록 처리            	  
                if(cellProperties && cellProperties.length > 0){
                	// 셀 복사일 경우 붙여 넣기 할 영역이 복사한 영역보다 넓은지 확인 한다.
                	// 붙여 넣기 할 영역이 넓을 경우 붙여 넣기 영역에 복사한 영역이 입력 가능한 만큼만 표시 할 수 있도록 처리 
                	if(cellProperties.length < (end.row - start.row+1) && cellProperties[r].length < (end.col - start.col+1)){
                		var maxrow = end.row - (end.row - start.row+1)%cellProperties.length;
                    	var maxcol = end.col - (end.col - start.col+1)%cellProperties[r].length;
                    	
                    	if(maxrow >= current.row && maxcol >= current.col){
                    		setCellData.push([current.row, current.col, cellProperties[r][c]]);
                    		setData.push([current.row, current.col, input[r][c]]);
                    	}
                	}else{
                		setCellData.push([current.row, current.col, instance.copyObject(cellProperties[r][c])]);
                		setData.push([current.row, current.col, input[r][c]]);
                	}
                }else{
                	setData.push([current.row, current.col, input[r][c]]);
          ...
          // jung 수정, 붙여 넣기 할 때 셀복사있지 텍스트 복사인지 구분하여 복사 하도록 처리 
          if(cellProperties && cellProperties.length > 0){
        	  // 선택된 영역에 끝점이 아닌 붙여 넣기 할 영역의 끝 점을 파라미터로 넘겨 주어야 한다.
        	  // 이는 병합된 셀이 영역에 포한 되어 있을 경우 정상 동작 되도록 처리하기 위함이다.
        	  current = {row : setCellData[setCellData.length-1][0], col : setCellData[setCellData.length-1][1]};
        	  
        	  if(instance.setDataAtProperties(setCellData, start, current, source || 'populateFromArray')){
  ...
  this.selection = selection = { //this public assignment is only temporary
    ...
    transformStart: function (rowDelta, colDelta, force) {
      ...
      // jung 추가, 키보드로 셀 이동 시 숨겨져 있는 셀을 건너 띄도록 추가
      var newCellProperties = instance.getCellMeta(coords.row, coords.col);
      if(newCellProperties.style.display == 'none'){
    	  var parentCellProperties = instance.getCellMeta(newCellProperties.data.row, newCellProperties.data.col);
		  var newPosition;
		  if(rowDelta < 0){
			  if(instance.getCustomSelected()[0] != parentCellProperties.row || parentCellProperties.row == 0){
				  coords.row = parentCellProperties.row;
			  }else{
				  coords.row = parentCellProperties.row - 1;
			  }
		  }else if(colDelta < 0){
			  if(instance.getCustomSelected()[1] != parentCellProperties.col || parentCellProperties.col == 0){
				  coords.col = parentCellProperties.col;
			  }else{
				  coords.col = parentCellProperties.col - 1;
			  }
		  }else if(rowDelta > 0){
			  newPosition = parseInt(parentCellProperties.rowSpan)+parentCellProperties.row;
			  if(newCellProperties.data.row == coords.row){
				  coords.row = parentCellProperties.row;
			  }else if(newPosition < instance.countRows()){
				  coords.row = parseInt(parentCellProperties.rowSpan)+parentCellProperties.row;
			  }
		  }else if(colDelta > 0){
			  newPosition = parseInt(parentCellProperties.colSpan)+parentCellProperties.col;
			  if(newCellProperties.data.col == coords.col){
				  coords.col = parentCellProperties.col;
			  }else if(newPosition < instance.countCols()){
				  coords.col = parseInt(parentCellProperties.colSpan)+parentCellProperties.col;
			  }
		  }        		  
	  }else if(newCellProperties.rowSpan > 1 || newCellProperties.colSpan > 1){
		  var customSelected = instance.getCustomSelected();
		  
		  if(rowDelta < 0 && customSelected[0] == newCellProperties.row && newCellProperties.row != 0){
			  coords.row = newCellProperties.row - 1;
		  }
		  
		  if(colDelta < 0 && customSelected[1] == newCellProperties.col && newCellProperties.col != 0){
			  coords.col = newCellProperties.col - 1;
		  }
	  }
      //jung 추가 end
      if (coords.row < 0) {
    ...
    empty: function () {
      ...
      var corners = instance.getCustomSelected();
      var r, c, changes = [];
      for (r = corners[0]; r <= corners[2]; r++) {
		  for (c = corners[1]; c <= corners[3]; c++) {
    ...
    /**
     * 선택된 영역에 셀 속성들을 초기화 한다.
     * @author JUNG(jgs135@naonsoft.com)
     */
    clearCellProperties : function(){
		if (!selection.isSelected()) {
		  return;
		}
		var corners = instance.getCustomSelected();
		...
		for (r = corners[0]; r <= corners[2]; r++) {
		  for (c = corners[1]; c <= corners[3]; c++) {
		    if (!instance.getCellMeta(r, c).readOnly) {
		    	changes.push([r, c, new priv.columnSettings[c]()]);
		    }
		  }
		}
		instance.setDataAtProperties(changes,null, null, 'edit');
	}
};
  ...
  editproxy = { //this public assignment is only temporary
    ...
    init: function () {
      priv.onCut = function onCut() {
        ...
        selection.empty();
        selection.clearCellProperties(); // jung 추가, 잘라내기 시 셀 속성도 같이 초기화 되도록 추가
      };

      priv.onPaste = function onPaste(str, event, cellProperties) {
        if (!instance.isListening() || !selection.isSelected()) {
          return;
        }

        var input = str//.replace(/^[\r\n]*/g, '').replace(/[\r\n]*$/g, '') //remove newline from the start and the end of the input(한줄 복사 시 오류가 발생하여 수정)
          , inputArray = SheetClip.parse(input)
          //, coords = grid.getCornerCoords([priv.selStart.coords(), priv.selEnd.coords()])
          , selected = instance.getCustomSelected()
          , areaStart = {row : selected[0], col : selected[1]}
          , areaEnd = {
            row: Math.max(selected[2], inputArray.length - 1 + selected[0]),
            col: Math.max(selected[3], inputArray[0].length - 1 +  selected[1])
          };
        ...
        for(var r=0; r < inputArray.length; r++){
        	for(var c=0; c < inputArray[r].length; c++){
        		if(inputArray[r][c].indexOf('id') > -1){
            		var tmpEl = document.createElement('DIV');
            		tmpEl.innerHTML = inputArray[r][c]; 
            		var dupleCheckMap = {};
            		var groupIdDuplMap = {};
            		var DateObj=new Date();
            		
            		$(tmpEl).find('[data-column-id]').each(function(){
            			$(this).attr('id', 'tmpId_' + String(DateObj.getTime())+Math.round(Math.random()*9999));
            			var decodeStr = Base64Encoder.decode($(this).data('option'));
    					var jsonObj = $.parseJSON(decodeURIComponent(decodeStr));
    					
    					jsonObj.columnId = String(DateObj.getTime())+Math.round(Math.random()*9999);
    					
    					if(jsonObj.items && jsonObj.items.length > 0){
							for(var k=0; k < jsonObj.items.length; k++){
								jsonObj.items[k].colItemId = String(DateObj.getTime())+Math.round(Math.random()*9999);
							}
						}
    					
            			$(this).attr('data-option', Base64Encoder.encode(encodeURIComponent(naon.json.getJSONString(jsonObj))));
            			$(this).attr('data-column-id', jsonObj.columnId);
            		});
            		inputArray[r][c] = $(tmpEl).html();
            	}
        	}
        }

        grid.populateFromArray(areaStart, inputArray, areaEnd, 'paste', priv.settings.pasteMode, cellProperties);
      };
      ...
      function onKeyDown(event) { 
        ...
        instance.isArea = true; // jung 추가, 문서 상에 스프레드 시트 바깥쪽에서 이벤트가 발생 했을 경우 에디터에 포커스가 가지 않도록 처리 하기 위해 추가
        ...
        //jung 추가
        if (event.ctrlKey && (event.keyCode === 67 || event.keyCode === 88)){	// ctrl+c, ctrl+x 
        	editproxy.setCopyableProperties();
        }else{
        	if(event.keyCode != 16){	// 라인 선택 div가 화면에 표시되고 shift키를 누른 다면 영역 선택으로 판단 한다. 
        		instance.view.wt.selections.area.border.lineSelectArea.style.display = 'none';
            	$(instance.view.wt.wtTable.TABLE).find('th').removeClass('th_select');	// th에 선택표시를 제거 한다.
            	instance.view.wt.wtTable.instance.selections['area'].isLineAll = false;
            	instance.view.wt.wtTable.instance.selections['current'].isLineAll = false;
        	}
        }
        //jung 추가 end
          ...
          if (!event.isImmediatePropagationStopped()) {
            ...
            switch (event.keyCode) {
              ...
              case 13: /* return/enter */
            	//jung 수정, 작성 시 엔터를 입력 하면 줄바꿈이 되도록 주석 처리함
        ...
        // jung 추가, 에디터 툴바에 표시하기 위한 함수
        instance.setTollbarEmphasize(instance.getCell(priv.selStart.row(), priv.selStart.col()));
      }
    ...
    setCopyableText: function () {
      var area = instance.getCustomSelected();
      var startRow = Math.min(area[0], area[2]);
      var startCol = Math.min(area[1], area[3]);
      var endRow = Math.max(area[0], area[2]);
      var endCol = Math.max(area[1], area[3]);
     ...
     * @author JUNG(jgs135@naonsoft.com)
     */
    setCopyableProperties: function () {
    	var area = instance.getCustomSelected();
        var startRow = Math.min(area[0], area[2]);
        var startCol = Math.min(area[1], area[3]);
        var endRow = Math.max(area[0], area[2]);
      ...
      instance.copyPaste.copyProperties(
    		  datamap.getCellProperties({row: startRow, col: startCol}, {row: finalEndRow, col: finalEndCol}),
    		  datamap.getText({row: startRow, col: startCol}, {row: finalEndRow, col: finalEndCol})
      );
    },
    ...
    prepare: function () {
      ...
      //for 한영 toggle
      $('.jqte_editor').css('opacity', 0);         
      $('.jqte_editor').css('filter', 'alpha(opacity=0)');
      $('.jqte_editor').css('width', '0px');
      $('.jqte_editor').css('height', '0px !important');
      $('.jqte_editor').css('fontSize', '0px');
      $('.jqte_editor').css('minHeight', '0px');
      $('.jqte_editor').css('fontFamily', 'sans-serif');
      if(instance.isArea){// jung 추가, 문서 상에 스프레드 시트 바깥쪽에서 이벤트가 발생 했을 경우 에디터에 포커스가 가지 않도록 처리 하기 위해 추가
    	  $('.handsontableInputHolder').show();
    	  setTimeout(this.focus, 10);
      }
    },
    
    /**
     * Sets focus to textarea(for 한영 toggle)
     */
    focus: function () {    	
  ...
  this.init = function () {
    ...
    // jung 추가, 로드 시 셀에 어트리뷰트 정보도 입력 하기 위해 추가
    if(priv.settings['attrData']){
    	this.setCellAttribute(priv.settings['attrData']);
    }
    // jung 추가 end
   ...
   * @param {String} source String that identifies how this change will be described in changes array (useful in onChange callback)
   */
  this.setDataAtProperties = function (row, start, end, source) {
	  var i,j, changes = [], cellProperties, area, rowIndex, colIndex;
	  
	  if(start && end){
		  // 붙여 넣기 할 영역에 병합된 셀이 있는지 확인 하여 복사한 영역이 병합된 셀을 포함 할 수 있는 지를 판단하여
		  // 포함 할 수 없을 경우 alert를 띄어 복사 할 수 없음을 사용자에게 알려 준다.
		  var isBreak = false;
		  for(i=start.row; i <= end.row; i++){
			  for(j = start.col; j <= end.col; j++){
				  cellProperties = instance.getCellMeta(i,j);
				  
				  if(cellProperties.colSpan > 1){
					  area = j+parseInt(cellProperties.colSpan)-1;
					  if(area > end.col || area < start.col){
						  naon.ui.alert({message : '복사 할 수 없습니다.'});
						  isBreak = true;
						  break;
					  }
				  }
				  ...
				  if(cellProperties.rowSpan > 1){
					  area = i+parseInt(cellProperties.rowSpan)-1;
				  ...
				  if(cellProperties.data){
					  if(cellProperties.data.row > end.row || cellProperties.data.row < start.row || 
							  cellProperties.data.col > end.col || cellProperties.data.col < start.col){
						  naon.ui.alert({message : '복사 할 수 없습니다.'});
						  isBreak = true;
						  break;
					  }
	  ...
	  for (i = 0; i < row.length; i++) {
		  cellProperties = instance.getCellMeta(row[i][0],row[i][1]);
		  var orgCellProperties = this.copyObject(cellProperties);
		  cellProperties.style = row[i][2].style;
		  cellProperties.colSpan = row[i][2].colSpan;
		  cellProperties.rowSpan = row[i][2].rowSpan;
		  
		  if(row[i][2].isNotCopy){
			  naon.ui.alert({message : '복사 할 수 없습니다.'});
			  return false;
		  }
		  //왼쪽 셀에 오른쪽 테두리가 칠해져 있으면 복사 할 셀 옆에 셀에도 테두리를 칠해 준다.
		  if(row[i][2].col > 0){			  
			  var leftCellProperties = instance.getCellMeta(row[i][2].row,row[i][2].col-1);
			  
			  if(leftCellProperties.style && leftCellProperties.style.borderRight && leftCellProperties.style.borderRight != ''){
				  if(row[i][1] > 0){
					  var orgCellProp = this.copyObject(priv.cellSettings[row[i][0]][row[i][1]-1]);
					  priv.cellSettings[row[i][0]][row[i][1]-1].style.borderRight = leftCellProperties.style.borderRight;
					  changes.push([ row[i][0], row[i][1]-1, orgCellProp, priv.cellSettings[row[i][0]][row[i][1]-1] ]);
				  }else{
					  cellProperties.style.borderLeft = leftCellProperties.style.borderRight;
				  }
			  }
		  }
		  
		  //위쪽 셀에 아래쪽 테두리가 칠해져 있으면 복사 할 셀 위에 셀에도 테두리를 칠해 준다.
		  if(row[i][2].row > 0){
			  var topCellProperties = instance.getCellMeta(row[i][2].row-1,row[i][2].col);
			  
			  if(topCellProperties.style && topCellProperties.style.borderBottom && topCellProperties.style.borderBottom != ''){
				  if(row[i][0] > 0){
					  var orgCellProp = this.copyObject(priv.cellSettings[row[i][0]-1][row[i][1]]);
					  priv.cellSettings[row[i][0]-1][row[i][1]].style.borderBottom = topCellProperties.style.borderBottom;
					  changes.push([ row[i][0]-1, row[i][1], orgCellProp, priv.cellSettings[row[i][0]-1][row[i][1]] ]);
				  }else{
					  cellProperties.style.borderTop = topCellProperties.style.borderBottom;
				  }
				   
			  }
		  }
		  
		  // 행에 첫번째 또는 열에 첫번째 셀을 복 사 할때 위쪽과 왼쪽에 테두리가 표시 되어 있을 수 있다. 다른 셀에 복사 시
		  // 위쪽 테두리와 왼쪽 테두리가 표시 되지 않도록 지운다.
		  if(cellProperties.style){
			  if(row[i][1] > 0 && cellProperties.style.borderLeft && cellProperties.style.borderLeft != ''){
				  var orgCellProp = this.copyObject(priv.cellSettings[row[i][0]][row[i][1]- 1]);
				  priv.cellSettings[row[i][0]][row[i][1]- 1].style.borderRight = cellProperties.style.borderLeft;
				  changes.push([ row[i][0], row[i][1]-1, orgCellProp, priv.cellSettings[row[i][0]][row[i][1]- 1] ]);
				  cellProperties.style.borderLeft = '';
			  }
			  
			  if(row[i][0] > 0 && cellProperties.style.borderTop && cellProperties.style.borderTop != ''){
				  var orgCellProp = this.copyObject(priv.cellSettings[row[i][0] - 1][row[i][1]]);
				  priv.cellSettings[row[i][0] - 1][row[i][1]].style.borderBottom = cellProperties.style.borderTop;
				  changes.push([ row[i][0]-1, row[i][1], orgCellProp, priv.cellSettings[row[i][0] - 1][row[i][1]] ]);
				  cellProperties.style.borderTop = '';
			  }
		  }
		  
		  if(row[i][2].data){
			  rowIndex = row[i][0] - (row[i][2].row - row[i][2].data.row);
			  colIndex = row[i][1] - (row[i][2].col - row[i][2].data.col);
			  cellProperties.data = {row : rowIndex, col : colIndex};			  
		  }else if(cellProperties.data){
			  delete cellProperties.data;
		  }
		  
		  priv.cellSettings[row[i][0]][row[i][1]] = cellProperties;
		  changes.push([row[i][0],row[i][1],orgCellProperties,cellProperties]);
	  }
	    

	  instance.PluginHooks.run('afterChange', changes, source);	// 취소/재실행을 위해 변경 사항을 알려준다.
	  instance.view.wt.wtTable.draw(false);	// 스프레드 시트를 다시 그린다.
	  return true;
  };
  ...
  /**
   * 취소/ 재실행에서 사용되는 함수로 셀 속성 및 데이터를 이전으로 돌리는 작업을 한다.
   * @author JUNG(jgs135@naonsoft.com)
   * @public
   * @param {Number|Array} row or array of changes in format [[row, prop, value], ...]
   * @param {String} prop or source String
   ...
   * @param {String} source String that identifies how this change will be described in changes array (useful in onChange callback)
   */
  this.setPropertiesAtRowProp = function (row, prop, value, source) {
	    var i, changes = [];
	    if(source == 'undo'){
	    	for (i = row.length-1; i >= 0; i--) {
	    		if(typeof row[i][2] == 'string'){	    			
	    			changes.push([row[i][0],row[i][1],datamap.get(row[i][0],row[i][1]),row[i][2]]);
	    			datamap.set(row[i][0],row[i][1],row[i][2]);
	    		}else{
	    			var orgCellProperties = instance.getCellMeta(row[i][0],row[i][1]);
		    		priv.cellSettings[row[i][0]][row[i][1]] = row[i][2];
			    	changes.push([row[i][0],row[i][1],orgCellProperties,row[i][2]]);
	    		}
		    }
	    }else{
	    	for (i = 0; i < row.length; i++) {
	    ...
	    instance.PluginHooks.run('afterChange', changes, source);
		instance.view.wt.wtTable.draw(false);
  };
  ...
  this.listen = function () {
    ...
    if(coords && coords[0] == coords[2] && coords[1] == coords[3]){
    	this.setTollbarEmphasize(this.getCell(coords[0],coords[1]));
    }
  };
  ...
  this.clear = function () {
    selection.selectAll();
    selection.empty();
    selection.clearCellProperties();	// jung 추가, 그리드 초기화 시 셀 속성도 같이 초기화 되도록 추가
  };
  ...
  this.getCellMeta = function (row, col) {
    ...
    // jung 추가, cell properties에 내가 추가한 키들의 default 값을 설정 하는 부분
    if(!cellProperties.style){
    	cellProperties.style = {};
    	cellProperties.style.display = '';
    	cellProperties.style.borderLeft = null;
        cellProperties.style.borderRight = null;
        cellProperties.style.borderTop = null;
        cellProperties.style.borderBottom = null;
        //cellProperties.style.height = '22px';
        cellProperties.style.verticalAlign = 'middle';
        cellProperties.style.textAlign = 'left';
        cellProperties.style.fontFamily = '맑은 고딕';
    }
  ...
  this._getRowHeightFromSettings = function (row) {
  	  /*
		행의 높이를 사용자가 변경 했을 경우 manualRowHeights에 값을 저장하게 된다.
		행의 높이를 리턴 할 때 변경된 높이 값이 있을 경우 변경된 값을 리턴하고 없을 경우
		초기 설정 값을 리턴 하도록 변경 하였다.
      */
	  if(Object.prototype.toString.call(instance.manualRowHeights) === '[object Array]' && instance.manualRowHeights[row] !== void 0){
		  return instance.manualRowHeights[row];
	  }else if (Object.prototype.toString.call(priv.settings.rowHeights) === '[object Array]' && priv.settings.rowHeights[row] !== void 0) {
		  return priv.settings.rowHeights[row];
	  }
  };
  ...
  this.getRowHeight = function (row) {
    var response = {
      height: instance._getRowHeightFromSettings(row)
    };
    if (!response.height) {
    	response.height = 22;    	
    }
    instance.PluginHooks.run('afterGetRowHeight', row, response);
  ...
  /**
   * 셀 병합
   * 주의 : 셀이 추가 되었을 때 스크롤이 생기는데 스크롤을 이동해도 병합된 셀이 따라 다닌다. 왜그런지 아직 모름
   * @author JUNG(jgs135@naonsoft.com)
   * @public
   * @return {Object}
   */
  this.cellMerge = function () {
	  var selected = this.getCustomSelected();
	  var changes = [];
	  
	  if(selected){
		  var firstCellProperties = this.getCellMeta(selected[0],selected[1]);
		  var orgFirstCellProperties =  this.copyObject(firstCellProperties);
		  
		  for(var i = selected[2]; i >= selected[0]; i--){
			  for(var j = selected[3]; j >= selected[1]; j--){
				  if(i == selected[0] && j == selected[1]) continue;
				  
				  var cellProperties = this.getCellMeta(i,j);
				  var orgCellProperties = this.copyObject(cellProperties);
				  cellProperties.style.display = 'none';
				  cellProperties.data = {row : selected[0], col : selected[1]};
				  changes.push([i,j,orgCellProperties,this.copyObject(cellProperties)]);
				  //병합 할때 숨겨지는 셀에 데이터를 없앤다.
				  changes.push([i,j,datamap.get(cellProperties.row, cellProperties.col),'']);
				  datamap.set(cellProperties.row, cellProperties.col,'');
			  }
		  }
		  
		  if(selected[2] - selected[0] > 0){
			  firstCellProperties.rowSpan = selected[2] - selected[0] + 1; 
		  }
		  
		  if(selected[3] - selected[1] > 0){
			  firstCellProperties.colSpan = selected[3] - selected[1] + 1; 
		  }
		  
		  if(selected[2] - selected[0] > 0 || selected[3] - selected[1] > 0){
			  changes.push([firstCellProperties.row,firstCellProperties.col,orgFirstCellProperties,this.copyObject(firstCellProperties)]);
		  }

		  instance.PluginHooks.run('afterChange', changes, 'edit');
		  instance.view.wt.wtTable.draw(false);	// 그리드 부분을 다시 그린다.
		  this.deselectCell();	// 현재 선택 영역을 취소
		  this.selectCell(selected[0],selected[1],selected[0],selected[1]);	// 재선택
	  }
  };
   ...
   * 셀 분할 하는 함수
   */
  this.mergeCancel = function(){
	  var selected = this.getCustomSelected();
	 /* if(selected[3] == 0){
		  selected = instance.view.wt.selections.area.getCorners(); //
	  }*/
	  var properties, parentCoords, parentProperties, orgCellProperties, changes = [];
	  
	  if(selected){
		  var parents = [];
		  
		  for(var i = selected[0]; i <= selected[2]; i++){
			  for(var j = selected[1]; j <= selected[3]; j++){
				  properties = this.getCellMeta(i,j);
				  orgCellProperties = this.copyObject(properties);
				  
				  if(properties.colSpan > 1 || properties.rowSpan > 1){
					  parents.push(properties);
				  }
				  
				  if(properties.style.display === 'none'){
					  parentCoords = properties.data;
					  parentProperties = this.getCellMeta(parentCoords.row, parentCoords.col);
					  $.extend(properties.style, parentProperties.style);
					  properties.style.borderLeft = '';
					  properties.style.borderRight = '';
					  properties.style.borderTop = '';
					  properties.style.borderBottom = '';
					  
					  if(j == selected[1]){
						 if(parentProperties.style.borderLeft != ''){
							 properties.style.borderLeft = parentProperties.style.borderLeft;
						 } 
					  }
					  
					  if(i == selected[0]){
						 if(parentProperties.style.borderTop != ''){
							 properties.style.borderTop = parentProperties.style.borderTop;
						 } 
					  }
					  
					  if(j == selected[3]){
						 if(parentProperties.style.borderRight != ''){
							 properties.style.borderRight = parentProperties.style.borderRight;
						 } 
					  }
					  
					  if(i == selected[2]){
						 if(parentProperties.style.borderBottom != ''){
							 properties.style.borderBottom = parentProperties.style.borderBottom;
						 }else{
							 if(i < instance.countRows()){
								var underCellProperties = this.getCellMeta(i+1,j);
								
								if(underCellProperties.style.borderTop != ''){
									properties.style.borderBottom = underCellProperties.style.borderTop;
									underCellProperties.style.borderTop = '';
								}
							 }
						 } 
					  }
					  ...
					  changes.push([i,j,orgCellProperties,this.copyObject(properties)]);
				  }
			  }
		  }
		  
		  for(var i=0; i< parents.length; i++){
			  orgCellProperties = this.copyObject(parents[i]);
			  
			  if(parents[i].colSpan > 1){
				  parents[i].style.borderRight = '';
				  parents[i].colSpan = 1;
			  }
			  
			  if(parents[i].rowSpan > 1){
				  parents[i].style.borderBottom = '';
				  parents[i].rowSpan = 1;
			  }
			  
			  changes.push([parents[i].row,parents[i].col,orgCellProperties,this.copyObject(parents[i])]);
		  }
		  
		  instance.PluginHooks.run('afterChange', changes, 'edit');
		  instance.view.wt.wtTable.draw(false);	// 그리드 부분을 다시 그린다.
		  this.deselectCell();	// 현재 선택 영역을 취소
		  this.selectCell(selected[0],selected[1],selected[2],selected[3]);	// 재선택
	  }	  
  };
  
  /**
   * json object를 복사하여 리턴하는 함수이다.
   */
  ...
  this.getHtml = function(){
	  var table = $(instance.$table).clone();
	  
	  $(table).addClass('formlate_tb');
	  $(table).find('thead').remove();
	  // 행 높이 값 설정
	  $(table).find('tr').each(function(idx){
		  var th = $(this).children(':first');
		  th.next().css('height',th.css('height'));	// 저장 후 화면에 표시될 높이
		  th.next().attr('data-height',th.css('height'));	// 수정 시 셀에 높이 값을 설정 하는 값이된다.
		  
		  // 아래 로직은 첫번째 셀이 병합된 셀일 경우 지정한 높이 값이 저장 후 표시 되지 않아 추가함
		  // 셀이 숨겨져 있을 경우 병합된 셀이라고 판단
		  // 병합된 셀 중 화면에 표시되고 있는 셀을 찾아 숨겨진 셀에 높이 값을 더해준다.
		  if(th.next().css('display') == 'none'){
			  var cellSettings = instance.getCellMeta(idx,0);
			  var $parentTd = $(th.parent().parent().children()[cellSettings.data.row]).children().first().next();
			  $parentTd.css('height', ($parentTd.height()+th.height())+'px');
		  }
	  });
	  $(table).find('th').remove();
	  $(table).find('td').css('word-break', 'break-all');//1BYTE 문자개행 처리를 위하여  #117270
	  // 열 너비 값 설정
	  $(table).find('col').first().remove();	  
	  $(table).find('col').each(function(idx, val){
		  var size = instance.manualColumnWidths[idx];
		  if(size){
	  ...
	  table.css('margin','0');
	  /**
	   * 주석처리가 되어 있었으나 브라우저별 브라우저별 폼릿이 상이하게 표현되어 다시 추가함 #116640
	   * 주석처리된 원인은 확인되지 않
	   */
	  table.css('table-layout','fixed');
	  table.css('font-family',"'나눔고딕', NanumGothic, 'NanumGothicWeb', 돋움, Dotum, sans-serif");
	  table.css('font-size','12px');
	  table.css('color', '444');
	  table.css('vertical-align', 'top');
	  table.attr('cellpadding', $('#frmFormletEditor_cellPadding').val());
	  var div = $('<div></div>');
	  div.append(table);
  ...
  /**
   * 셀에 어트르뷰트 정보를 설정하는 함수
   */
  this.setCellAttribute = function(attrData){
	  var visibleRows = instance.countRows()
	    , visibleColumns = instance.countCols()
	    , r, c, i, j, colSpan, rowSpan, childCellProperties;
	  
	  for (r = 0; r < visibleRows; r++) {
		for (c = 0; c < visibleColumns; c++) {
			var cellProperties = this.getCellMeta(r,c);
			var attr = attrData[r][c];
			if(attr.colSpan){
				cellProperties.colSpan = attr.colSpan;
			}
			
			if(attr.rowSpan){
				cellProperties.rowSpan = attr.rowSpan;
			}
			
			if(attr.colSpan || attr.rowSpan){
				colSpan = attr.colSpan ? parseInt(attr.colSpan) : 1;
				rowSpan = attr.rowSpan ? parseInt(attr.rowSpan) : 1;
				for(i=r; i < r+rowSpan; i++){
					for(j=c; j < c+colSpan; j++){
						if(i == r && j == c){
							continue;
						}
						
						childCellProperties = this.getCellMeta(i,j);
						childCellProperties.data = {row : r, col : c};
					}
				}
			}
			
			if(attr.style){
				cellProperties.style = attr.style;				
			}
		}
	  }
  };
   ...
   * @author JUNG(jgs135@naonsoft.com)
   */
  this.cellOutLine = function(type, lineWeight, lineType, lineColor){
	  var styleVal = lineWeight + ' ' + lineType + ' ' + lineColor;
	  var selected = this.getCustomSelected();
	  ...
	  if(selected){
		  var parentCoords, parentCellProperties, orgParentCellProperties, orgCellProperties, changes = [];
		  //left, right
		  if(drawLeft || drawRight){
			  for(var i = selected[0]; i <= selected[2]; i++){
				  
				  if(drawRight){
					  var rightCellProperties = this.getCellMeta(i, selected[3]);
					  orgCellProperties = this.copyObject(rightCellProperties);
					  
					  if(rightCellProperties.style.display == 'none'){
						  parentCoords = rightCellProperties.data;
						  parentCellProperties = this.getCellMeta(parentCoords.row, parentCoords.col);
						  orgParentCellProperties = this.copyObject(parentCellProperties);
						  
						  parentCellProperties.style.borderRight = styleVal;
						  changes.push([parentCoords.row, parentCoords.col,orgParentCellProperties,this.copyObject(parentCellProperties)]);
					  }else{
						  rightCellProperties.style.borderRight = styleVal;
						  changes.push([i, selected[3],orgCellProperties,this.copyObject(rightCellProperties)]);
					  }
				  }
				  
				  if(drawLeft){
					  if(selected[1] > 0){
						  var leftCellProperties = this.getCellMeta(i,selected[1]-1);
						  orgCellProperties = this.copyObject(leftCellProperties);
						  
						  if(leftCellProperties.style.display == 'none'){
							  parentCoords = leftCellProperties.data;
							  parentCellProperties = this.getCellMeta(parentCoords.row, parentCoords.col);
							  orgParentCellProperties = this.copyObject(parentCellProperties);
							  ...
							  leftCellProperties.style.borderRight = styleVal;
							  changes.push([i,selected[1]-1,orgCellProperties,this.copyObject(leftCellProperties)]);
						  }
					  }else{
						  this.getCellMeta(i,selected[1]).style.borderLeft = styleVal;
						  orgCellProperties = this.copyObject(this.getCellMeta(i,selected[1]));
						  changes.push([i,selected[1],orgCellProperties,this.copyObject(this.getCellMeta(i,selected[1]))]);
					  }	//	end if
				  }	// end if
			  }	// end for
		  }	// end if
		  ...
		  if(drawTop || drawBottom){
			  for(var i = selected[1]; i <= selected[3]; i++){
				  
				  if(drawBottom){
					  var bottomCellProperties = this.getCellMeta(selected[2],i);
					  orgCellProperties = this.copyObject(bottomCellProperties);
					  if(bottomCellProperties.style.display == 'none'){
						  parentCoords = bottomCellProperties.data;
						  parentCellProperties = this.getCellMeta(parentCoords.row, parentCoords.col);
						  orgParentCellProperties = this.copyObject(parentCellProperties);
						  parentCellProperties.style.borderBottom = styleVal;
						  changes.push([parentCoords.row, parentCoords.col,orgParentCellProperties,this.copyObject(parentCellProperties)]);
					  }else{
						  bottomCellProperties.style.borderBottom = styleVal;
						  changes.push([selected[2],i,orgCellProperties,this.copyObject(bottomCellProperties)]);
					  }
				  }
				  
				  if(drawTop){
					  if(selected[0] > 0){
						  var topCellProperties = this.getCellMeta(selected[0]-1,i);
						  orgCellProperties = this.copyObject(topCellProperties);
						  if(topCellProperties.style.display == 'none'){
							  parentCoords = topCellProperties.data;
							  parentCellProperties = this.getCellMeta(parentCoords.row, parentCoords.col);
							  orgParentCellProperties = this.copyObject(parentCellProperties);
							  parentCellProperties.style.borderBottom = styleVal;
							  changes.push([parentCoords.row, parentCoords.col,orgParentCellProperties,this.copyObject(parentCellProperties)]);
						  }else{
							  topCellProperties.style.borderBottom = styleVal;
							  changes.push([selected[0]-1,i,orgCellProperties,this.copyObject(topCellProperties)]);
						  }
					  }else{
						  this.getCellMeta(selected[0],i).style.borderTop = styleVal;
						  orgCellProperties = this.copyObject(this.getCellMeta(selected[0],i));
						  changes.push([selected[0],i,orgCellProperties,this.copyObject(this.getCellMeta(selected[0],i))]);
					  }	// end if
				  }	// end if
			  }	// end for
		  }	//end if
   ...
   * @author JUNG(jgs135@naonsoft.com)
   */
  this.cellAllOutLine = function(lineWeight, lineType, lineColor){
	  var selected = this.getCustomSelected();
	  var parentCoords,parentCellProperties;
	  var styleVal;
	  if(!lineWeight || !lineType || !lineColor)
		  styleVal = '';
	  else
		  styleVal = lineWeight + ' ' + lineType + ' ' + lineColor;
	  ...
	  if(selected){
		  ...
		  for(var i = selected[0]; i <= selected[2]; i++){
			  for(var j = selected[1]; j <= selected[3]; j++){
				  ...
				  cellProperties.style.borderLeft = '';				 
				  cellProperties.style.borderTop = '';				  
				  cellProperties.style.borderRight = styleVal;
				  cellProperties.style.borderBottom = styleVal;
				  
				  if(j == selected[1]){
					  
					  if(j > 0){
						  var leftCellProperties = this.getCellMeta(i,j-1);
						  var orgLeftCellProperties = this.copyObject(leftCellProperties);
						  leftCellProperties.style.borderRight = styleVal;
						  changes.push([i,j-1,orgLeftCellProperties,this.copyObject(leftCellProperties)]);
						  
						  if(leftCellProperties.style.display == 'none'){
							  parentCoords = leftCellProperties.data;
							  parentCellProperties = this.getCellMeta(parentCoords.row, parentCoords.col);
							  var orgParentCellProperties = this.copyObject(parentCellProperties);
							  parentCellProperties.style.borderRight = styleVal;
							  changes.push([parentCoords.row, parentCoords.col,orgParentCellProperties,this.copyObject(parentCellProperties)]);
						  }
					  }else{
						  cellProperties.style.borderLeft = styleVal;
					  }
				  }
				  
				  if(i == selected[0]){
					  if(i > 0){
						  var topCellProperties = this.getCellMeta(i-1,j);
						  var orgTopCellProperties = this.copyObject(topCellProperties);
						  topCellProperties.style.borderBottom = styleVal;
						  changes.push([i-1,j,orgTopCellProperties,this.copyObject(topCellProperties)]);
						  
						  if(topCellProperties.style.display == 'none'){
							  parentCoords = topCellProperties.data;
							  if(selected[1] == parentCoords.col){
								  parentCellProperties = this.getCellMeta(parentCoords.row, parentCoords.col);
								  
								  if(selected[3] >= parentCellProperties.colSpan){
									  var orgParentCellProperties = this.copyObject(parentCellProperties);
									  parentCellProperties.style.borderBottom = styleVal;
									  changes.push([parentCoords.row, parentCoords.col,orgParentCellProperties,this.copyObject(parentCellProperties)]);
								  }
							  }else{
								  cellProperties.style.borderTop = styleVal;  
							  }
						  }
					  }else{
						  cellProperties.style.borderTop = styleVal;
  ...
  /**
   * 현재 선택한 영역의 좌표를 리턴하는 함수
   */
  this.getCustomSelected = function () { //https://github.com/warpech/jquery-handsontable/issues/44  //cjl
	if (selection.isSelected()) {
		// jung 추가
		// 테두리 표시 시 셀 병합때문에 변경된 선택 영역을 잘못 가지고 와서 수정 코드를 삽입 했습니다.
		var visibleRows = instance.countRows()
	    , visibleColumns = instance.countCols()
	    , minCol = visibleColumns, minRow = visibleRows, maxCol=0, maxRow=0;
	  
		for (var r = 0; r < visibleRows; r++) {
			for (var c = 0; c < visibleColumns; c++) {
		  	  var cell = instance.getCell(r,c);
		  	  if($(cell).hasClass('area') || $(cell).hasClass('current')){
		  		 if(minCol > c){
		  			 minCol = c;
		  		 } 
  ...
  /**
   * 선택된 셀 내용에 스타일 적용하는 함수
   * @author JUNG(jgs135@naonsoft.com)
   */
  this.setCellInsideStyle = function(style){	  
	  var selected = this.getCustomSelected();
	  ...
	  if(selected){
		  ...
		  for(var i = selected[2]; i >= selected[0]; i--){
			  for(var j = selected[3]; j >= selected[1]; j--){
				  ...
				  if(cell){
					  ...
					  $.each(style,function(jsonKey,jsonVal){
						  switch (jsonKey) {
							case 'bold':
								data = data.replace(/<b>/gi,"").replace(/<\/b>/gi,"");
								...
								break;
							case 'underline':
								data = data.replace(/<u>/gi,"").replace(/<\/u>/gi,"");
								...
								break;
							case 'strikeThrough':
								data = data.replace(/<strike>/gi,"").replace(/<\/strike>/gi,"");
								...
								break;
							case 'italic':			
								data = data.replace(/<i>/gi,"").replace(/<\/i>/gi,"");
							...
							default:
								...
								$(div).find('[style^="'+jsonKey+'"]').each(function(){
									$(this).css(jsonKey,'');
								});
								
								$(div).find('[style=""]').each(function(){
									$(this).before($(this).html());
									$(this).remove();
								});
								data = '<span style="'+jsonKey+':'+jsonVal+';">' + $(div).html() + '</span>';
								break;
						  }						     
					  });
		  ...
		  this.setDataAtCell(rows, null, null, 'edit');
	  }
  };
  
  /**
   * 선택된 셀에 정렬 방식 변경 하는 함수
   * @author JUNG(jgs135@naonsoft.com)
   */
	  ...
	  if(selected){
		  ...
		  for(var i = selected[2]; i >= selected[0]; i--){
			  for(var j = selected[3]; j >= selected[1]; j--){
				  var cellProperties = this.getCellMeta(i,j);
				  var orgCellProperties = this.copyObject(cellProperties);
				  $.extend(cellProperties.style,style);
  ...
  /**
   * 에디터 툴바에 현재 적용 되어 있는 스타일을 표시 할 수 있도록 처리 하는 하무
   * @param TD {Object} 셀 엘리먼트
   * @author JUNG(jgs135@naonsoft.com)
   */
  this.setTollbarEmphasize = function(TD){
	var td = $(TD);
	var fullText = td.text();
	
	//글꼴과 글자 크기 초기화
	var fFamily = $('#frmFormletEditor_fFamily_ul').find('[name="font"]:first');
	var fSize = $('#frmFormletEditor_fSize_ul').find('[name="font_size"]:first');
	fFamily.prop('checked', true);
	fSize.prop('checked', true);
	$('[data-command="fFamily"]').text(fFamily.val());
	$('[data-command="fSize"]').text(fSize.val());
	//글꼴과 글자 크기 초기화 end
	$('.toolbar').find("a.tool").each(function(){
	...
	if(td.find('span').length > 0){
		...
		td.find('span').each(function(){
			if(!fontSize && this.style && this.style.fontSize){
				fontSize = this.style.fontSize;
			}else if(!fontFamily && this.style && this.style.fontFamily){
		...
		if(fontFamily){//폰트
			$('#frmFormletEditor_fFamily_ul').find('[name="font"]').each(function(){
				if(this.value == fontFamily){
					fFamily.prop('checked', true);
					$('[data-command="fFamily"]').text(fontFamily);
					return false;
				}
			});
		}
		...
		if(fontSize){//크기
			$('#frmFormletEditor_fSize_ul').find('[name="font_size"]').each(function(){
				if(this.value == fontSize){
					fSize.prop('checked', true);
					$('[data-command="fSize"]').text(fontSize);
					return false;
				}
			});
		}
	...
	if(td.find('b').length > 0){
		if(fullText == $(td.find('b')[0]).text()){
			$('.toolbar').find(".bold").addClass('active');
		}
	}
	
	if(td.find('i').length > 0){
		if(fullText == $(td.find('i')[0]).text()){
			$('.toolbar').find(".italic").addClass('active');
		}
	}  
	
	if(td.find('u').length > 0){
		if(fullText == $(td.find('u')[0]).text()){
			$('.toolbar').find(".underline").addClass('active');
		}
	}

	if(td.find('strike').length > 0){
		if(fullText == $(td.find('strike')[0]).text()){
			$('.toolbar').find(".strikeThrough").addClass('active');
		}
	}
	
	if(td.css('text-align') != ''){
		var align = td.css('text-align');
		
		if(align == 'left'){
			$('.toolbar').find(".justifyLeft").addClass('active');
		}else if(align == 'center'){
			$('.toolbar').find(".justifyCenter").addClass('active');
		}else if(align == 'right'){
			$('.toolbar').find(".justifyRight").addClass('active');
		}
	}
	
	if(td.css('vertical-align') != ''){
		var align = td.css('vertical-align');
		
		if(align == 'top'){
			$('.toolbar').find(".topAlign").addClass('active');
		}else if(align == 'middle'){
			$('.toolbar').find(".middleAlign").addClass('active');
		}else if(align == 'bottom'){
			$('.toolbar').find(".bottomAlign").addClass('active');
		}
	}
  };
  
  /**
   * 에디터 툴바에 셀을 선택 했을 때와 에디팅 시에 동작을 구분하기 위한 이벤트를 등록 해제 하는 함수이다.
   * @param action {String} 이벤트 등록/해제 여부
   * @author JUNG(jgs135@naonsoft.com)
   */
  this.toolbarEnventHandle = function(action){
	  var that = this;
	  if(action == 'on'){
		  $('.jqte_editor').removeClass('focused');
		  $('#frmFormletEditor_insertorderedlist').addClass('disabled');
		  $('#frmFormletEditor_insertunorderedlist').addClass('disabled');
		  $('.specialChar').addClass('disabled');
		  $('.outdent').addClass('disabled');
		  $('.indent').addClass('disabled');
		  
		  $('#frmFormletEditor_lineHeight').removeClass('disabled');
		  $('#frmFormletEditor_cellHandle_div').find('a.tool, .select_code').removeClass('disabled');
		  $('#frmFormletEditor_sort_div').find('a.tool, .select_code').removeClass('disabled');	
		  $('#frmFormletEditor_cellPadding').attr('disabled', false);
		  $('#frmFormletEditor_undoRedo_div').find('a.tool, .select_code').removeClass('disabled');
		  $('#frmFormletEditor_save_div').find('a.tool, .select_code').removeClass('disabled');
		  
		  
	  }else{
		  $('.jqte_editor').addClass('focused');
		  $('#frmFormletEditor_insertorderedlist').removeClass('disabled');
		  $('#frmFormletEditor_insertunorderedlist').removeClass('disabled');
		  $('.specialChar').removeClass('disabled');
		  $('.outdent').removeClass('disabled');
		  $('.indent').removeClass('disabled');
		  
		  $('#frmFormletEditor_lineHeight').addClass('disabled');
		  $('#frmFormletEditor_cellHandle_div').find('a.tool, .select_code').addClass('disabled');
		  $('#frmFormletEditor_sort_div').find('a.tool, .select_code').addClass('disabled');
		  $('#frmFormletEditor_cellPadding').attr('disabled', true);
		  $('#frmFormletEditor_undoRedo_div').find('a.tool, .select_code').addClass('disabled');
		  $('#frmFormletEditor_save_div').find('a.tool, .select_code').addClass('disabled');
	  }
  };
  /**
   * 셀 사이즈를 설정 하는 레이어를 띄어 준다.
   */
  this.resizingSell = function(command, size){
      ...
      if(command == 'col'){
    	  var beforeColumnWidth = $.extend([], instance.manualColumnWidths);
    	  for(var i = selection.start.col(); i <= selection.end.col(); i++){
    		  instance.manualColumnWidths[i] = size;
    	  }
    	  
    	  instance.forceFullRender = true;
          instance.view.render(); //updates all
          //instance.PluginHooks.run('persistentStateSave', 'manualColumnWidths', instance.manualColumnWidths);
          instance.PluginHooks.run('afterColumnResize', beforeColumnWidth, instance.manualColumnWidths);
      }else if(command == 'row'){
    	  var beforeRowHeight = $.extend([], instance.manualRowHeights);
    	  for(var i = selection.start.row(); i <= selection.end.row(); i++){
    		  instance.manualRowHeights[i] = size;
    	  }
    	  
    	  instance.forceFullRender = true;
          instance.view.render(); //updates all
          // object copy
          var menualHeights = $.extend([],instance.manualRowHeights);
          instance.PluginHooks.run('afterRowResize', beforeRowHeight, menualHeights);
      }
  };
  
  this.getCoords = function(TD){
	  return instance.view.wt.wtTable.getCoords(TD);
  };
...
$.fn.handsontable = function (action) {
  ...
  if (typeof action !== 'string') {
    ...
    if (instance) {
      ...
      // jung 추가, 로드 시 셀에 어트리뷰트 정보도 입력 하기 위해 추가
      if(userSettings.attrData){
    	instance.setCellAttribute(userSettings.attrData);
      	instance.view.wt.wtTable.draw(false);
      }
      // jung 추가 end
    }
...
Handsontable.TableView = function (instance) {
  ...
  $(table).attr('cellpadding', $('#frmFormletEditor_cellPadding').val());// jung 추가, 기본 세팅
  table.style.fontSize = '9pt';// jung 추가, 기본 세팅
  ...
  $documentElement.on('mousedown.' + instance.guid, function (event) {
    ...
    // jung 추가, 문서 상에 스프레드 시트 바깥쪽에서 이벤트가 발생 했을 경우 에디터에 포커스가 가지 않도록 처리 하기 위해 추가
    if(next !== that.wt.wtTable.spreader){
    	$('.handsontableInputHolder').hide();
    	instance.isArea = false;
    }
 // jung 추가 END
  ...
  var walkontableConfig = {
    ...
    onCellMouseDown: function (event, coords, TD) {
      that.wt.wtTable.instance.selections.area.border.lineSelectArea.style.display = 'none';
      $(that.wt.wtTable.TABLE).find('th').removeClass('th_select'); // th에 선택표시를 제거 한다.
      that.wt.wtTable.instance.selections['area'].isLineAll = false;
      that.wt.wtTable.instance.selections['current'].isLineAll = false;
      instance.listen();
      ...
      if (event.button === 2 && instance.selection.inInSelection(coordsObj)) { //right mouse button
        ...
        instance.setTollbarEmphasize(TD);
      }
  ...
  this.wt = new Walkontable(walkontableConfig, instance);
...
// jung 수정, 셀 내용 입력 시 	에디트 영역에서도 셀렉션이 빠졌다고 표시 되는 것을 막기 위해 추가
	
  if ($(el).closest('.handsontableInput').length > 0 || $(el).hasClass('handsontableInput')) {
    return (true);
  }
// jung 수정 end
...
Handsontable.TableView.prototype.applyCellTypeMethod = function (methodName, td, row, col) {
  ...
  return method(this.instance, td, row, col, prop, this.instance.getDataAtRowProp(row, col), cellProperties);
};
...
Handsontable.helper.isPrintableChar = function (keyCode) {
	if(navigator.userAgent.toLowerCase().indexOf('firefox') > -1){
		return ((keyCode == 32) || //space
			    ...
			    keyCode >= 226 || keyCode == 0 || //special chars (229 for Asian chars)
...
HandsontableTextEditorClass.prototype.createElements = function () {
  ...
  this.TEXTAREA = $('.jqte_editor')[0];	//jung 수정, 에디터 적용
  //this.TEXTAREA.contentEditable = true;	// jung 추가
  this.textareaStyle = this.TEXTAREA.style;
  this.textareaStyle.width = 0;
  this.textareaStyle.minHeight = 0;	//jung 수정, textarea의 경우 height값을 저의 하지만 div는 minHeight값을 정의 하도록 변경함
  //this.textareaStyle.backgroundColor = '#fff';// jung 추가
  this.textareaStyle.fontSize = '9pt'; // jung 추가, 에디터 영역에 기본 폰트 사이즈값
  this.textareaStyle.imeMode = 'active';
  this.$textarea = $(this.TEXTAREA);
  this.$textarea.addClass('handsontableInput');	// jung 추가, 기존에 클래스를 설정하는 부분을 추가 하도록 수정 함 
  this.$textarea.show();	// jung 추가, 에디터를 적용하면 로드시 display 속성이 none으로 되어 있는 에디터를 block을 변경 함
  ...
  this.isModify = false;	// jung 추가, 에디팅 시에 방향키가 입력되면 셀이 이동 하지 않도록 처리하기 위해 추가
...
HandsontableTextEditorClass.prototype.bindEvents = function () {
  ...
  this.$textarea.off('.editor').on('keydown.editor', function (event) {
	    ...
	    //jung 수정, 화살표와 엔터 키 입력 시  작성이 종료 되는 것을 막음
	    switch (event.keyCode) {
	      case 38: /* arrow up */
	    	  if(that.isModify){
	    		  event.stopImmediatePropagation();
	      ...
	      case 13: /* return/enter */
	        ...
	        if ((event.ctrlKey && !isMultipleSelection) || event.altKey) { //if ctrl+enter or alt+enter, add new line
	          // jung 수정, alt+enter 시 개행 되도록 수정
	          //var pNode = document.createElement('P');
	          
	          //pNode.appendChild(textNode);
	          //that.TEXTAREA.appendChild(pNode);
	        	if($.browser.msie){
    	    		var range = document.selection.createRange();
    	    		//range.pasteHTML("<br><span></span>");
    	    		range.pasteHTML("<br> ");
    	    		...
    	    		/*if($(container).hasClass('jqte_editor')){
    	    			range.insertNode(span);
    	    			range.insertNode(br);
    	    		}else{
    	    			container = $(container).closest('.jqte_editor');
    	    			$(container).append(br).append(span);
    	    		}*/
    	    		//selection을 확인하여 노드의 위치를 재조정 한다.
    	    		if(!selection.isCollapsed){
    	    			range.setStartAfter(range.endContainer,0);
        	    		range.setEndAfter(range.endContainer,0);
    	    		}
    	    		...
    	    		if(range.commonAncestorContainer.nodeType == 3){
    	    			if(range.commonAncestorContainer.length == range.startOffset){
	          ...
	          /*that.TEXTAREA.innerHTML = that.TEXTAREA.innerHTML + '\n';
	          that.TEXTAREA.focus();*/
	          event.stopImmediatePropagation();
	        }
	        else {
...
HandsontableTextEditorClass.prototype.bindTemporaryEvents = function (td, row, col, prop, value, cellProperties) {
  ...
  function onDblClick() {
    that.TEXTAREA.innerHTML = that.originalValue;
    that.instance.destroyEditor();
    that.beginEditing(row, col, prop, true, undefined, true); //show edit field
  ...
  // 셀 선택  시 해당 셀이 머지된 셀이고 숨겨져 있는 셀일 경우 부모 셀에 데이터를 originalValue에 넣는다.
  // 이 값은 작성 취소 시에 사용된다.
  if(cellProperties.data){
	  this.originalValue = that.instance.getDataAtRowProp(cellProperties.data.row,cellProperties.data.col);
  }
...
HandsontableTextEditorClass.prototype.beginEditing = function (row, col, prop, useOriginalValue, suffix, isModify) {
	this.instance.toolbarEnventHandle('off');// jung 추가 , 에디터 툴바를 사용 하기 위한 이벤트 제어
	$('.jqte_editor').addClass('focused');
	// jung 추가, 데이터 작성 시 수정일 경우 방향키 이벤트로 셀이 이동 하지 않도록 처리하기 위해 추가한 코드
	var scrollLeft = parseInt($('#fd_canvas').scrollLeft());
	
	if(isModify !== true){
	  this.isModify = false;
	  $('#fd_canvas').off('scroll');
	}else{
	  this.isModify = isModify;
	  $('#fd_canvas').on('scroll', function(e){$('#fd_canvas')[0].scrollLeft = scrollLeft;});
	}
	// jung 추가 end
	
	// jung 추가, 방향키로 셀 이동 후 작성 시 병합된 셀로 이동한 경우 위치를 잡지 못하는 경우가 발생 하여 추가하게 되었습니다.
	var cellProperties = this.instance.getCellMeta(row, col);
	
	if(cellProperties.style.display == 'none'){
		row = cellProperties.data.row;
		col = cellProperties.data.col;
	}
	// jung 추가 end
	...
	//for 한영 toggle 
	$('.jqte_editor')[0].style.opacity = 100;         
	$('.jqte_editor')[0].style.filter = "alpha(opacity=100)";
	...
	if (useOriginalValue) {
	  this.TEXTAREA.innerHTML = Handsontable.helper.stringify(this.originalValue) + (suffix || '');
	}
	else {
	...
	this.TEXTAREA.focus();
	this.setCaretPosition(this.TEXTAREA, this.TEXTAREA.innerHTML.length);
...
HandsontableTextEditorClass.prototype.refreshDimensions = function () {
  ...
  var width = $td.width()
    ...
    , height = $td.height()	// jung 수정, 기존 코드 ($td.outerHeight() - 4)을 변경 - edit div 높이를 셀 높이와 같게 하기 위해 수정 
  ...
  // jung 추가 editor에 셀 스타일을 적용 하기위해 추가
  var cell = this.instance.getCell(this.row,this.col);
  $(this.TEXTAREA).css('text-align',$(cell).css('text-align'));
  $(this.TEXTAREA).css('line-height',$(cell).css('line-height'));
  
  if($(cell).css('background-color') != ''){
	  $(this.TEXTAREA).css('background-color',$(cell).css('background-color'));
  }
//jung 추가 end
  
  this.textareaStyle.width = Math.min(width, maxWidth)+'px';	//jung 추가, edit div를 띄울때 width값을 변경
  this.textareaStyle.minHeight = Math.min(height, maxHeight)+'px';	//jung 추가, edit div를 띄울때 minHeight값을 변경
  this.textareaStyle.height = '';
  this.textareaStyle.fontSize = '';
  this.textareaStyle.fontFamily = '';
  // jung 추가, 셀 여백과 같이 div에도 여백을 주기 위해 테이블에 cellpadding 정보를 읽어와 div에 padding값으로 넣어 준다.
  if(this.instance.$table.attr('cellpadding') && this.instance.$table.attr('cellpadding') != ''){
	  var px = parseInt(this.instance.$table.attr('cellpadding'));
	  px = px > 0 ? px-1 : px; 
	  this.textareaStyle.padding = px + 'px';
  }
...
HandsontableTextEditorClass.prototype.finishEditing = function (isCancelled, ctrlDown) {
	...
	if (this.state == this.STATE_EDITING) {
	  ...
	  this.instance.toolbarEnventHandle('on');// jung 추가, 에디터 툴바를 사용 하기 위한 이벤트 제어
	  $('.jqte_editor').removeClass('focused');
	  $('#fd_canvas').off('scroll'); //작성 중에 스크롤이 되지 않도록 등록 한 이벤트 제거
...
HandsontableAutocompleteEditorClass.prototype.bindTemporaryEvents = function (td, row, col, prop, value, cellProperties) {
  ...
  function onDblClick() {
    that.beginEditing(row, col, prop, true, undefined, true);
...
HandsontableHandsontableEditorClass.prototype.bindTemporaryEvents = function (td, row, col, prop, value, cellProperties) {
  ...
  var options = {
    ...
    afterOnCellMouseDown: function () {
      ...
      parent.TEXTAREA.innerHTML = this.getDataAtCell(sel[0], sel[1]);
...
(function(Handsontable){
  ...
  function createContextMenu() {
    var instance = this
      ...
      //TODO 다국어 처리 필요
      , allItemsRow = {
        "row_above": {name: "위에 행 삽입", disabled: isDisabled},
        "row_below": {name: "아래에 행 삽입", disabled: isDisabled},
        "repeat_set": {name: "반복 설정", disabled: isDisabled},
        "remove_repeat": {name: "반복 취소", disabled: isDisabled},
        "repeat_groupId": {name: "반복 아이디 조회", disabled: isDisabled},
        "hsep3": "---------",
        "remove_row": {name: "선택한 행 삭제", disabled: isDisabled},
        "hsep2": "---------",
        "row_resize": {name: "높이 조정", disabled: isDisabled}
      }
      , allItemsCol = {
            "col_left": {name: "왼쪽에 열 삽입", disabled: isDisabled},
            "col_right": {name: "오른쪽에 열 삽입", disabled: isDisabled},
            "hsep2": "---------",
            "remove_col": {name: "선택한 열 삭제", disabled: isDisabled},
            "hsep3": "---------",
            "col_resize": {name: "너비 조정", disabled: isDisabled}
      }
      , allItemsCell = {
            "undo": {name: "실행취소", disabled: isDisabled},
            "redo": {name: "다시 실행", disabled: isDisabled}
      }
      , defaultOptions = {
        selector : "#" + selectorId + ' table th',
        trigger  : 'right',
    ...
    function onContextClick(key) {
      ...
      var rowAmount = (selection.end.row() - selection.start.row()) + 1; // 열,행 추가 시 사용
      var colAmount = (selection.end.col() - selection.start.col()) + 1; // 열,행 추가 시 사용
      
      switch (key) {
        case "row_above":
          instance.alter("insert_row", selection.start.row(),rowAmount);
          break;

        case "row_below":
          instance.alter("insert_row", selection.end.row() + 1,rowAmount);
          break;

        case "col_left":
          instance.alter("insert_col", selection.start.col(),colAmount);
          break;

        case "col_right":
          instance.alter("insert_col", selection.end.col() + 1,colAmount);
          break;

        case "remove_row":
          instance.alter(key, selection.start.row(), rowAmount);
          break;

        case "remove_col":
          instance.alter(key, selection.start.col(), colAmount);
          break;
        ...
        case "repeat_set":
            ...
            $(instance.view.wt.wtTable.TBODY).find('.th_select').each(function(){
            	$(this).parent().attr('data-repeat-group-id', groupId);
            	$(this).parent().addClass('tr_repeat');
            	$(this).addClass('th_repeat');
            });
            break;
        case "remove_repeat":
        	$(instance.view.wt.wtTable.TBODY).find('.th_select').each(function(){
            	$(this).parent().removeAttr('data-repeat-group-id');
            	$(this).parent().removeClass('tr_repeat');
            	$(this).removeClass('th_repeat');
            });
            break;
        case "repeat_groupId":
        	$('#frmFormEditor_repeatId_lyr_groupId').html($(this).parent().data('repeatGroupId'));
        	openDialog('frmFormEditor_repeatId_lyr');        	
            break;
        case 'col_resize' :
        	
        	if(selection.end.col() - selection.start.col() == 0){
        		var TH = $(this)[0];      	  
        	    ...
        	    $('#frmFormEditor_col_size_lyr_input').val(Math.min(thWidth, maxWidth));
        	}else{
        		$('#frmFormEditor_col_size_lyr_input').val('');
        	}
        	openDialog('frmFormEditor_col_size_lyr');
        	$('#frmFormEditor_col_size_lyr_input').focus();
        	$("#frmFormletEditor_spreadsheet").handsontable('unlisten');
        	break;
        case 'row_resize' :
        	if(selection.end.row() - selection.start.row() == 0){
        		var height = $(this)[0].style.height;      	  
        	    $('#frmFormEditor_row_size_lyr_input').val(parseInt(height));
        	}else{
        		$('#frmFormEditor_row_size_lyr_input').val('');
        	}
        	openDialog('frmFormEditor_row_size_lyr');
        	$('#frmFormEditor_row_size_lyr_input').focus();
        	$("#frmFormletEditor_spreadsheet").handsontable('unlisten');
        	break;
      }
    }
   ...
   /*if (settings.contextMenu === true) { //contextMenu is true
    ...
    options.items = allItemsRow;
    options.selector = "#" + selectorId + ' table tbody > tr > th';
    $.contextMenu($.extend(true, {}, defaultOptions, options));
    
    options.items = allItemsCol;
    options.selector = "#" + selectorId + ' table thead > tr > th';
    $.contextMenu($.extend(true, {}, defaultOptions, options));
    
    options.items = allItemsCell;
    options.selector = "#" + selectorId + ' table tbody > tr > td';
    $.contextMenu($.extend(true, {}, defaultOptions, options));
  }

  function destroyContextMenu() {
    var id = this.rootElement[0].id;
    $.contextMenu('destroy', "#" + id + " table tbody > tr > th");
    $.contextMenu('destroy', "#" + id + ' table thead > tr > th');
    $.contextMenu('destroy', "#" + id + ' table tbody > tr > td');
  }
...
function HandsontableManualColumnResize() {
  var pressed
    , currentTH
    , currentCol
    , currentRow	//jung 추가, row 사이즈 변경 때문에 추가
    , currentWidth
    , currentHeight	//jung 추가, row 사이즈 변경 때문에 추가
    , instance
    , newSize
    , startX
    , startY	//jung 추가, row 사이즈 변경 때문에 추가
    , startWidth
    , startHeight //jung 추가, row 사이즈 변경 때문에 추가
    , startOffset
    , resizer = document.createElement('DIV')
    , rowResizer = document.createElement('DIV')	//jung 추가, row 사이즈 변경 때문에 추가
    , handle = document.createElement('DIV')
    , rowHandle = document.createElement('DIV')	//jung 추가, row 사이즈 변경 때문에 추가
    , line = document.createElement('DIV')
    , rowLine = document.createElement('DIV')	//jung 추가, row 사이즈 변경 때문에 추가
    , lineStyle = line.style
    , rowLineStyle = rowLine.style	//jung 추가, row 사이즈 변경 때문에 추가
    , isCol = true	//jung 추가, row 사이즈 변경 때문에 추가
    , isMousePressed, firstTH, isSelectedRowLine;
  ...
  //jung 추가, row 사이즈 변경 때문에 추가
  rowResizer.className = 'manualRowResizer';
  rowHandle.className = 'manualRowResizerHandle';
  rowResizer.appendChild(rowHandle);
  rowLine.className = 'manualRowResizerLine';
  rowResizer.appendChild(rowLine);
  //jung 추가 end
  var beforeChangeWidth, beforeChangeHeight;	// 사이즈 변경 전 사이즈를 저장 할 변수
  var $document = $(document);

  $document.mousemove(function (e) {
    if (pressed) {
    	if(instance.view.wt.selections.area.border.lineSelectArea.style.display == 'block'){
    		var selected = instance.getSelected();
    		
    		if(isCol){
    			if(isSelectedRowLine && selected[1] <= currentCol &&  selected[3] >= currentCol){// 왼쪽에서 오른쪽으로 드래그
        			currentWidth = startWidth + (e.pageX - startX);
        			
    				for(var i=selected[1]; i <= selected[3]; i++){
    					if(currentCol == i) continue;
    					setManualSize(i, currentWidth);
    				}
    			}else if(isSelectedRowLine && selected[3] <= currentCol &&  selected[1] >= currentCol){// 오른쪽에서 왼쪽으로 드래그
        			currentWidth = startWidth + (e.pageX - startX);
        			
    				for(var i=selected[3]; i <= selected[1]; i++){
    					if(currentCol == i) continue;
    					setManualSize(i, currentWidth);
    				}
    			}
    		}else{
    			if(!isSelectedRowLine && selected[0] <= currentRow &&  selected[2] >= currentRow){// 위에서 아래로 드래그
    				currentHeight = startHeight + (e.pageY - startY);
        			
    				for(var i=selected[0]; i <= selected[2]; i++){
    					if(currentRow == i) continue;
    					setRowManualSize(i, currentHeight-3);
    				}
    			}else if(!isSelectedRowLine && selected[2] <= currentRow &&  selected[0] >= currentRow){// 아래에서 위로 드래그
    				currentHeight = startHeight + (e.pageY - startY);
    	...
    	if(isCol){
    		...
    		currentHeight = startHeight + (e.pageY - startY);
    		newSize = setRowManualSize(currentRow, currentHeight-3); //save col width
    		rowResizer.style.top = startOffset + currentHeight + 'px';
    	}
    }
  });
    	...
    	if(isCol){
    		instance.view.wt.wtDom.removeClass(resizer, 'active');
    	      if(newSize != startWidth){
    	    	  ...
    	    	  var menualWidths = $.extend([],instance.manualColumnWidths);// object copy
    	    	  instance.PluginHooks.run('afterColumnResize', beforeChangeWidth, menualWidths);
    	    	  beforeChangeWidth = undefined;
    	      }

    	      refreshResizerPosition.call(instance, currentTH);
    	}else{
    		instance.view.wt.wtDom.removeClass(rowResizer, 'active');
	  	    
	  	    if(newSize != startHeight){
	  	    	instance.forceFullRender = true;
    	        instance.view.render(); //updates all
    	        var menualHeights = $.extend([],instance.manualRowHeights);// object copy
	  	    	instance.PluginHooks.run('afterRowResize', beforeChangeHeight, menualHeights);
	  	    	beforeChangeHeight = undefined;
	  	    }
	  	    
	  	    refreshRowResizerPosition.call(instance, currentTH);
   ...
   * @param TH
   */
  function refreshRowResizerPosition(TH) {
	    instance = this;
	    currentTH = TH;

	    var row = this.view.wt.wtTable.getCoords(TH)[0]; //getCoords returns array [row, col]
	    if (row >= 0) { //if not row header
	      ...
	      startOffset = (thOffset - rootOffset) - 6;
	      var thStyle = this.view.wt.wtDom.getComputedStyle(TH);
	      rowResizer.style.top = startOffset + parseInt(this.view.wt.wtDom.outerHeight(TH), 10) + 'px';

	      this.rootElement[0].appendChild(rowResizer);
	    }
  }
   ...
   * @author JUNG(jgs135@naonsoft.com)
   */
  function refreshRowLinePosition() {
    var instance = this;
    var thBorderHeight = 2 * parseInt(this.view.wt.wtDom.getComputedStyle(currentTH).borderHeight, 10);
    startHeight = parseInt(this.view.wt.wtDom.outerHeight(currentTH), 10);
    instance.view.wt.wtDom.addClass(rowResizer, 'active');
    rowLineStyle.width = this.getSettings().width + 'px';
    pressed = instance;
  }
  
  /**
   * 테이블 해더 부분을 클릭 할 경우 해당 라인 전체가 선택 되도록 하는 함수
   * @param TH
   */
  function selectLineHighlight(TH){
	  instance = this;
	  currentTH = TH;
	  var coords = this.view.wt.wtTable.getCoords(TH); //getCoords returns array [row, col]
	  //셀 병합해제시 오류로 인하여 제거 함 #116107
	  //this.view.wt.wtTable.instance.selections['area'].isLineAll = true;
	  //this.view.wt.wtTable.instance.selections['current'].isLineAll = true;
	  
	  if(firstTH){
		  var firstCoords = this.view.wt.wtTable.getCoords(firstTH);
		  var itemList, startIdx, endIdx;
		  
		  if(coords[0] === -1){
			  isSelectedRowLine = true;
			  this.selectCell(0,firstCoords[1],(this.view.wt.wtTable.rowStrategy.cellCount-1),coords[1]);
			  // 라인선택을 할 경우 현재 선택된 라인에 해더에 색깔을 변경 해주기 위한 로직이다.
			  itemList = $(firstTH).parent().children();
			  startIdx = firstCoords[1] + 1;
			  endIdx = coords[1] + 1;
			  
			  if(firstCoords[1] > coords[1]){
				startIdx = coords[1] + 1;
				endIdx = firstCoords[1] + 1;
			  }
			  for(var i in itemList){
				  if(i >= startIdx && i <= endIdx){
					  $(itemList[i]).addClass('th_select');//선택 표시 클래스  
				  }
			  }
		  }else{
			  isSelectedRowLine = false;
			  this.selectCell(firstCoords[0],0,coords[0],(this.view.wt.wtTable.columnStrategy.cellCount-1));
			  
			  // 라인선택을 할 경우 현재 선택된 라인에 해더에 색깔을 변경 해주기 위한 로직이다.
			  itemList = $(firstTH).closest('tbody').children();
			  startIdx = firstCoords[0];
			  endIdx = coords[0];
			  
			  if(firstCoords[0] > coords[0]){
				startIdx = coords[0];
				endIdx = firstCoords[0];
			  }
			  for(var i in itemList){
				  if(i >= startIdx && i <= endIdx){
					  $(itemList[i]).find('th').addClass('th_select');//선택 표시 클래스  
				  }
			  }
		  } 
	  }else{
		  if(coords[0] === -1){ 
			  isSelectedRowLine = true;
			  this.selectCell(0,coords[1],(this.view.wt.wtTable.rowStrategy.cellCount-1),coords[1]);
		  }else{
			  isSelectedRowLine = false;
			  this.selectCell(coords[0],0,coords[0],(this.view.wt.wtTable.columnStrategy.cellCount-1));
		  }
		  
		  $(currentTH).addClass('th_select');//선택 표시 클래스
	  }
  }
  ...
  var bindManualColumnWidthEvents = function () {
    ...
    this.rootElement.on('mouseenter.handsontable', 'th', function (e) {
      if (!pressed) { //리사이즈 중이 아닐 경우
    	...
    	if($(this).find('div').length > 0){
    		...
    		isCol = false;
    		refreshRowResizerPosition.call(instance, e.currentTarget);
    	}
        
    	if(isMousePressed){
    		$(instance.view.wt.wtTable.TABLE).find('th').removeClass('th_select'); //기존에 선택 되어 있는 열을 해제
    		selectLineHighlight.call(instance, e.currentTarget);
    	}else{
    ...
    // jung 추가, 테이블 해더 부분을 클릭 할 경우 해당 라인 전체가 선택 되도록 추가
    this.rootElement.on('mousedown.handsontable', 'th', function (e) {
    	if (!pressed) {
    		if(e.button == 2 && $(e.currentTarget).hasClass('th_select')) return false;
    		
    		if(!firstTH || !e.shiftKey){
    			firstTH = e.currentTarget;
    		}
    		
    		isMousePressed = true;
    		instance.isArea = true;	// jung 추가, 문서 상에 스프레드 시트 바깥쪽에서 이벤트가 발생 했을 경우 에디터에 포커스가 가지 않도록 처리 하기 위해 추가
    		$(instance.view.wt.wtTable.TABLE).find('th').removeClass('th_select'); //기존에 선택 되어 있는 열을 해제
    		instance.view.wt.selections.area.border.lineSelectArea.style.display = 'block';
    		selectLineHighlight.call(instance, e.currentTarget); 
    	}
    });
    // jung 추가 end
    ...
    this.rootElement.on('mousedown.handsontable', '.manualColumnResizer', function () {
      if (autoresizeTimeout == null) {
        autoresizeTimeout = setTimeout(function () {
          if (dblclick >= 2 && instance.determineColumnWidth) { // 리사이즈 바 위에서 더블 클릭 했을 경우 자동 사이즈 맞춤을 해주기 위한 이벤트 처리
            newSize = instance.determineColumnWidth.call(instance, currentCol);
            setManualSize(currentCol, newSize);
            instance.forceFullRender = true;
            instance.view.render(); //updates all
            var menualWidths = $.extend([],instance.manualColumnWidths);
            instance.PluginHooks.run('afterColumnResize', instance.manualColumnWidths, menualWidths);
          }
          dblclick = 0;
    ...
    this.rootElement.on('mousedown.handsontable', '.manualColumnResizer', function (e) {
      ...
      newSize = startWidth;
      beforeChangeWidth = $.extend([],instance.manualColumnWidths); // 사이즈 변경 중일 경우 변경 전 사이즈를 저장
    });
    
    //jung 추가, 사이즈 변경 때문에 추가	
	this.rootElement.on('mousedown.handsontable', '.manualRowResizer', function (e) {
	    startY = e.pageY;
	    refreshRowLinePosition.call(instance);
	    newSize = startHeight;
	    beforeChangeHeight = $.extend([],instance.manualRowHeights); // 사이즈 변경 중일 경우 변경 전 사이즈를 저장
	});
	//jung 추가 end
	
	$document.on('mouseup.handsontable', function(e){
		isMousePressed = false;
		//firstTH = undefined;
	});
  };
  ...
  this.beforeInit = function () {
    ...
    this.manualRowHeights = [];	// 행 높이 변경 정보를 담을 객체 jgs135
  };
  ...
  this.init = function (source) {
    ...
    if (manualColumnWidthEnabled) {
      ...
      if (typeof loadedManualColumnWidths != 'undefined') {
    	...
    	// 수정 시 너비 값이 있을 경우 너비 값을 기록하는 manualColumnWidths에 설정 한다.  
        this.manualColumnWidths = initialColumnWidths;
      } else {
        this.manualColumnWidths = [];
      }
      // 수정 시 높이 값이 있을 경우 높이 값을 기록하는 manualRowHeights에 설정 한다.
      var initialRowHeights = this.getSettings().rowHeights; 
      if(initialRowHeights instanceof Array){
    	  this.manualRowHeights = this.getSettings().rowHeights; 
      }else{
  ...
  var setManualSize = function (col, width) {
    ...
    // jung 추가
    // col 사이즈 변경 시 스크롤이 생기지 않도록 옆에 있는 col에 사이즈에 늘어난 만큼 줄여준다.
    /*if(isCol){
    	if(col == instance.getSettings().startCols-1){
    		return instance.getColWidth(col);
    	}
    	
    	if(!instance.manualColumnWidths[col]){
        	instance.manualColumnWidths[col] = instance.getColWidth(col);  
        }
        
        if(!instance.manualColumnWidths[col+1]){
        	instance.manualColumnWidths[col+1] = instance.getColWidth(col+1);  
        }
        
        var maxWidth = instance.manualColumnWidths[col] + instance.manualColumnWidths[col+1] - 20;
        //console.log(instance.manualColumnWidths[col]);
        //console.log(instance.manualColumnWidths[col+1]);
    	
    	if(maxWidth < width){
    		width = maxWidth;
    	}
    	
    	var interval =  width - instance.manualColumnWidths[col];    	
    	instance.manualColumnWidths[col+1] = instance.manualColumnWidths[col+1] - interval;
    }*/
    // jung 추가 end
  ...
  /**
   * 높이가 변경되면 높이 값을 저장하기 위해 호출 하는 함수
   * @author jgs135
   */
  var setRowManualSize = function (row, height) {
  ...
  /**
   * 파라미터로 받은 행에 사이즈가 변경되었는지 확인 하고 변경된 값을 리턴한다.
   * @author jgs135
   */
  this.getRowHeight = function (row, response) {
    if (this.getSettings().manualColumnResize && this.manualRowHeights[row]) {
      response.height = this.manualRowHeights[row];
    }
  };
}
...
//PluginHooks.run 을 이용하여 함수를 호출 하기 위해 등록 처리 해준다.
Handsontable.PluginHooks.add('afterGetRowHeight', htManualColumnResize.getRowHeight);
...
(function(Handsontable){
  Handsontable.UndoRedo = function (instance) {
    ...
    instance.addHook("beforeRemoveRow", function (index, amount) {
      ...
      var cellMetaList = new Array();
      
      // 병합, 테두리 정보도 같이 저장 하도록 함
      var start = index > 0 ? index-1 : index;
      var end = index+amount == plugin.instance.countVisibleRows() -1 ? index+amount : index+amount+1;
      for(var i=start; i <= end; i++){
    	  var cols = new Array();
    	  for(var j=0; j < plugin.instance.countVisibleCols(); j++){
    		  cols.push($.extend(true, {}, plugin.instance.getCellMeta(i, j))); 
    	  }
    	  cellMetaList.push(cols);
      }
      // 행 높이값을 저장
      var rowHeight = new Array();
      for(var i=index; i < index+amount; i++){
    	  if(plugin.instance.manualRowHeights[i]){
    		  rowHeight.push(plugin.instance.manualRowHeights[i]);
    	  }else{
    		  rowHeight.push(undefined);
    	  }
      }
      var action = new Handsontable.UndoRedo.RemoveRowAction(index, removedData, cellMetaList, rowHeight);
      plugin.done(action);
    });
    ...
    instance.addHook("beforeRemoveCol", function (index, amount) {
      ...
      // 병합, 테두리 정보도 같이 저장 하도록 함
      var start = index > 0 ? index-1 : index;
      var end = index+amount == plugin.instance.countVisibleCols() -1 ? index+amount : index+amount+1;
      for(var i=0; i < plugin.instance.countVisibleRows(); i++){
    	  var rows = new Array();
    	  for(var j=start; j <= end; j++){
    		  rows.push($.extend(true, {}, plugin.instance.getCellMeta(i, j)));
    	  }
    	  cellMetaList.push(rows);
      }      
      // 행 높이값을 저장
      var colWidths = new Array();
      for(var i=index; i < index+amount; i++){
    	  if(plugin.instance.manualColumnWidths[i]){
    		  colWidths.push(plugin.instance.manualColumnWidths[i]);
    	  }else{
    		  colWidths.push(undefined);
    	  }
      }
      
      var action = new Handsontable.UndoRedo.RemoveColumnAction(index, removedData, cellMetaList, colWidths);
      plugin.done(action);
    });
    
    // afterColumnResize 호출 시 실행 할 함수 선언
    instance.addHook("afterColumnResize", function (beforeChangeWidth, manualColumnWidths) {
        var action = new Handsontable.UndoRedo.ColumnResizeAction(beforeChangeWidth, manualColumnWidths);
        plugin.done(action);
      });
    
    // afterRowResize 호출 시 실행 할 함수 선언
    instance.addHook("afterRowResize", function (beforeChangeHeight, manualRowHeights) {
        var action = new Handsontable.UndoRedo.RowResizeAction(beforeChangeHeight, manualRowHeights);
        plugin.done(action);
     });
  };
  ...
  Handsontable.UndoRedo.ChangeAction.prototype.undo = function (instance) {
    ...
    if(data.length > 0){
	    if(data[0][2] != null && typeof data[0][2] == 'object'){
	    	instance.setPropertiesAtRowProp(data, null, null, 'undo');
	    }else{
  ...
  Handsontable.UndoRedo.ChangeAction.prototype.redo = function (instance) {
    ...
    if(data.length > 0){
	    if(data[0][2] != null && typeof data[0][2] == 'object'){
	    	instance.setPropertiesAtRowProp(data, null, null, 'redo');
	    }else{
  ...
  Handsontable.UndoRedo.RemoveRowAction = function (index, data, cellMeta, rowHeight) {
    this.index = index;
    this.amount = data.length;
    this.data = data;	// 행에 입력된 데이터 배열    
    this.cellMeta = cellMeta;	// 삭제된 행들의 셀 정보 배열
    this.rowHeight= rowHeight;	// 삭제된 행들의 높이 값 배열
  };
  ...
  Handsontable.UndoRedo.RemoveRowAction.prototype.undo = function (instance) {
    ...
    // 실행 취소 시 행 높이값 원복 하는 부분
    for(var i=0; i < this.rowHeight.length; i++){
    	if(this.rowHeight[i]){
    		instance.manualRowHeights.splice(this.index+i,0,this.rowHeight[i]);
    	}
    }
    instance.render();
    
    // 실행 취소 시 병합, 테두리가 사라지는 오류로 인하여 추가됨
    var cellMeta = new Array();
    for(var i=0; i < this.cellMeta.length; i++){
    	for(var j=0; j < this.cellMeta[i].length; j++){
    		var meta = this.cellMeta[i][j];
    		cellMeta.push([meta.row, meta.col, $.extend(true, {}, meta)]);
    	}
    }
    instance.setPropertiesAtRowProp(cellMeta, null, null, 'undo');
  };
  ...
  Handsontable.UndoRedo.RemoveColumnAction = function (index, data, cellMeta, colWidths) {
    this.index = index;
    this.data = data;	// 삭제된 열에 입력된 데이터 배열
    this.amount = this.data[0].length;
    this.cellMeta = cellMeta;	// 삭제된 열의 셀 정보 배열
    this.colWidths= colWidths;	// 삭제된 열의 너비 배열
  };
  ...
  Handsontable.UndoRedo.RemoveColumnAction.prototype.undo = function (instance) {
    ...
    for(var i=0; i < this.colWidths.length; i++){
    	if(this.colWidths[i]){
    		instance.manualColumnWidths[this.index+i] = this.colWidths[i];
    	}
    }
    instance.render();
    // 실행 취소 시 병합, 테두리가 사라지는 오류로 인하여 추가됨
  ...
  // 리사이즈 실행 취소, 재 실행 함수 
  Handsontable.UndoRedo.ColumnResizeAction = function (beforeChangeWidth, manualColumnWidths) {
	    this.beforeChangeWidth = beforeChangeWidth;
	    this.manualColumnWidths = manualColumnWidths;
  };
  Handsontable.helper.inherit(Handsontable.UndoRedo.ColumnResizeAction, Handsontable.UndoRedo.Action);
  Handsontable.UndoRedo.ColumnResizeAction.prototype.undo = function (instance) {
	  instance.manualColumnWidths = $.extend([], this.beforeChangeWidth);
	  instance.forceFullRender = true;
      instance.view.render(); //updates all
  };
  Handsontable.UndoRedo.ColumnResizeAction.prototype.redo = function (instance) {
	  instance.manualColumnWidths = $.extend([], this.manualColumnWidths);
	  instance.forceFullRender = true;
      instance.view.render(); //updates all
  };
  
  Handsontable.UndoRedo.RowResizeAction = function (beforeChangeHeight, manualRowHeights) {
	  this.beforeChangeHeight = beforeChangeHeight;
	  this.manualRowHeights = manualRowHeights;
  };
  Handsontable.helper.inherit(Handsontable.UndoRedo.RowResizeAction, Handsontable.UndoRedo.Action);
  Handsontable.UndoRedo.RowResizeAction.prototype.undo = function (instance) {
	  instance.manualRowHeights = $.extend([],this.beforeChangeHeight);// object copy
	  instance.forceFullRender = true;
	  instance.view.render(); //updates all
  };
  Handsontable.UndoRedo.RowResizeAction.prototype.redo = function (instance) {
	  instance.manualRowHeights = $.extend([],this.manualRowHeights);// object copy
	  instance.forceFullRender = true;
	  instance.view.render(); //updates all
  };
})(Handsontable);
...
function CopyPasteClass() {
  ...
  this.cutCallbacks = [];
  this.pasteCallbacks = [];
  this.copyCellProperties = [];	//jung 추가, 셀 복사 시에 속성도 같이 복사되도록 처리 하기 위해 생성
  this.copyText = [];	//jung 추가, 셀 복사 인지 일반 텍스트 복사인지를 판단하기 위해 텍스트를 저장하기 위해 생성
...
/**
 * 복사된 셀들에 속성 및 데이터를 저장한다.
 */
CopyPasteClass.prototype.copyProperties = function (cellProperties, str) {
  this.copyCellProperties = cellProperties;
  this.copyText = str;
};
...
CopyPasteClass.prototype.triggerPaste = function (event, str) {
  ...
  if (that.pasteCallbacks) {
    setTimeout(function () {
      // jung 추가, 셀 속성도 같이 복사 됐을 경우 복사 시점에 저장한 텍스트와 현재 복사된 텍스트가 같은지 비교 하여 
      // 셀 속성 정보를 넘기도록 한다. 이유는 외부에서 복사한 내용을 붙여넣기 할 때 이전에 복사되어 있는 셀 속성 정보가 같이
      // 적용 되지 않도록 하기 위함이다.	
      var properties, isOnlyLine, val;

      if(that.copyText == (str || that.elTextarea.value)){
    	  properties = that.copyCellProperties;
      }
	  
	  val = (str || that.elTextarea.value).replace(/\n$/, ''); //remove trailing newline
	  
	  // 세로로 머지된 셀을 붙여넣기 할 때 영역이 잘 못 잡히는 오류가 발생하여 일단 주석처리함
	  // 추가적으로 아래 코드가 어떤 상황 때문에 추가한 코드인지 잘 모르겠음 다시 확인 필요
      /*if(typeof(that.copyText) == 'string' && that.copyText.substring(that.copyText.length-2, that.copyText.length) == '\n\n'){
    	  isOnlyLine = true;  
      }

      if(isOnlyLine){
    	  if(that.copyText.replace(/\n$/, '') == (str || that.elTextarea.value)){
        	  properties = that.copyCellProperties;
          }  
    	  
    	  val = (str || that.elTextarea.value);
      }else{
    	  if(that.copyText == (str || that.elTextarea.value)){
        	  properties = that.copyCellProperties;
          }
      ...
      for (var i = 0, ilen = that.pasteCallbacks.length; i < ilen; i++) {
        that.pasteCallbacks[i](val, event, properties); // jung 추가, properties 파라미터 추가
      }
    }, 50);
  }
};
...
//  style.visibility = 'hidden';
  ...
  var lineSelectArea = document.createElement('DIV');
  var lineSelectAreaStyle = lineSelectArea.style;
  lineSelectArea.className = 'selection';
  lineSelectAreaStyle.display = 'none';
  lineSelectAreaStyle.top = 0;
  lineSelectAreaStyle.left = 0;
  lineSelectAreaStyle.position = 'absolute';
  lineSelectAreaStyle.opacity = '0.5';
  lineSelectAreaStyle.filter = 'alpha(opacity=50)';
  lineSelectAreaStyle.zIndex = '5';
  lineSelectAreaStyle.backgroundColor = 'rgb(160, 195, 255)';
  this.main.appendChild(lineSelectArea);
  this.lineSelectArea = lineSelectArea;
  
  $(this.lineSelectArea).on('mousedown.selection', function (e) {
	  $(this).hide();
 ...
 * @param {Array} corners
 */
WalkontableBorder.prototype.appear2 = function (corners) {
  ...
  if(corners[1] == 0 && corners[3] == instance.wtTable.columnStrategy.cellCount-1){
	  firstTH = instance.wtTable.getCell([corners[0], -1]);
	  fromOffset = this.wtDom.offset(firstTH);
	  top = fromOffset.top - containerOffset.top;
	  left = fromOffset.left - containerOffset.left + this.wtDom.outerWidth(firstTH);
	  width = parseInt($(instance.wtTable.TBODY).css('width')) - this.wtDom.outerWidth(firstTH);
	  height = 0;
	  
	  for(var i = corners[0]; i <= corners[2]; i++){
		  firstTH = instance.wtTable.getCell([i, -1]);
		  height += this.wtDom.outerHeight(firstTH);
	  }	  
  }else{
	  firstTH = $(instance.wtTable.THEAD).find('th')[corners[1]+1];
	  endTH = $(instance.wtTable.THEAD).find('th')[corners[3]+1];
	  fromOffset = this.wtDom.offset(firstTH);
	  top = fromOffset.top - containerOffset.top + this.wtDom.outerHeight(firstTH);
	  left = fromOffset.left - containerOffset.left;
	  height = parseInt($(instance.wtTable.TBODY).css('height')) -2;
	  width = this.wtDom.outerWidth(firstTH);
	  
	  while(firstTH !== endTH){
		  firstTH = $(firstTH).next()[0];
		  width += this.wtDom.outerWidth(firstTH); 
	  }	  
  }
  ...
  this.lineSelectArea.style.top = top + 'px';
  this.lineSelectArea.style.left = left + 'px';
  this.lineSelectArea.style.width = width + 'px';
  this.lineSelectArea.style.height = height + 'px';  
};
...
WalkontableBorder.prototype.appear = function (corners) {
  ...
  if (fromRow !== void 0 && fromColumn !== void 0) {
    ...
    if(toTD.style.display == 'none'){
    	toTD = instance.wtTable.getCell([$(toTD).data('mergeCell').row, $(toTD).data('mergeCell').col]);
    }
  ...
  // jung 추가, 한영변환으로 인하여 에디터에 항상 포커스가 가게 됩니다. 에디터는 마지막으로 작성한 셀 위치에 있다가 셀 데이터를 작성하려고 할때 해당 셀 위로 이동합니다.
  // 페이지에 스크롤이 생겼을 경우 에디터가 위치한 곳으로 자동으로 이동하게 되는 오류가 있어 이를 수정하기위해 추가 합니다.
  $('.handsontableInputHolder').css('top',top + 'px');
  $('.handsontableInputHolder').css('left',left + 'px');
  // jung 추가 END
...
// Walkontable 안에서 Core에 접근하기 위해 파라미터 추가
function Walkontable(settings, coreInstance) {
  var that = this,
    originalHeaders = [];
...
WalkontableDom.prototype.index = function (elem) {
  var i = 0;
  //jung 추가, 사이즈 변경 때문에 추가
  // 첫번째 행에 index와 두번째 행에 index가 같아서 추가한 코드
  if(elem.parentNode.tagName == 'THEAD'){
	  i=-1;
  }
  //jung 추가 end
  ...
  /*else if (this.instance.wtTable.isColumnAfterViewport(coords[1]) || (this.instance.wtTable.getLastVisibleColumn() === coords[1] && !this.instance.wtTable.isLastColumnFullyVisible())) {
    ...
    if (sum < available) {
      ...
      while (sum + next < available && scrollTo >= fixedColumnsLeft) {
    	next = this.instance.getSetting('columnWidth', scrollTo - 1);	//jung 추가, 스크롤 시 셀에 width가 일정하지 않을 경우 스크롤이 잘못 동작 하는것을 수정
        scrollTo--;
        sum += next;
      }
    }
    ...
    //no scroll 
	  // jung 추가, 선택된 셀에 이동에 따라 스크롤이 이동 되도록 추가
	  var scrollTo = this.instance.wtSettings.instance.selections.current.selected[0][1];
	  var sum = this.instance.getSetting('columnWidth', -1);
	  var available = parseInt($('#fd_canvas').css('width'));
	  var next = -1;
	  var bodyElement = $('#fd_canvas');
	  
	  while(available > sum){
		  ++next;
		  sum += this.instance.getSetting('columnWidth', next);
	  ...
	  var minSum = available - (available - bodyElement.scrollLeft());

	  if(next < scrollTo){
		  fixedColumnsLeft = scrollTo - next;
		  sum = 0;
		  for (var i = 1; i <= fixedColumnsLeft; i++) {
			  sum += this.instance.getSetting('columnWidth', next+i);
		  }
		  
		  if(bodyElement.scrollLeft() < sum){
			  bodyElement.scrollLeft(sum);
		  }else if(minSum > available+(sum - this.instance.getSetting('columnWidth', next+fixedColumnsLeft))){
			  bodyElement.scrollLeft(sum);
		  }
	  }else{
		  sum = 0;
		  for (var i = 0; i <= scrollTo; i++) {
			  sum += this.instance.getSetting('columnWidth', next+i);
		  }
...
WalkontableSelection.prototype.draw = function () {
  var corners, r, c, source_r, source_c, modifyCorners; //jung 추가 modifyCorners, 셀병합 시 선택 영역 정상 동작 하도록 하기 위해 추가
  ...
  if (this.selected.length) {
    ...
    modifyCorners= corners;	// jung 추가, 셀병합 시 선택 영역 정상 동작 하도록 하기 위해 추가
    ...
    for (r = 0; r < visibleRows; r++) {
      for (c = 0; c < visibleColumns; c++) {
        ...
        if (source_r >= corners[0] && source_r <= corners[2] && source_c >= corners[1] && source_c <= corners[3]) {
          // jung 추가
          // 병합된 셀이 포함되어 있을 경우 셀력션이 잘못 표시 되는 걸 보정 하기 위한 코드
          var cell = this.instance.wtTable.getCell([r,c]);
          
          if(cell && typeof cell == 'object'){
        	  if(!this.isLineAll){
	        	  if(cell.colSpan > 1 || cell.rowSpan > 1 || cell.style.display == 'none'){
	            	  var row = r;
	            	  var col = c;
	            	  
	            	  if(cell.style.display == 'none'){
	                	  var mergeCell = $(cell).data('mergeCell');
	                	  row = mergeCell.row;
	                	  col = mergeCell.col;
	                	  cell =  this.instance.wtTable.getCell([row,col]);
	                  }
	            	  
	            	  var minR = row > corners[0] ? corners[0] : row;
	            	  var maxR = row > corners[2] ? row : corners[2];
	            	  var minC = col > corners[1] ? corners[1] : col;
	            	  var maxC = col > corners[3] ? col : corners[3];
	            	  maxR = maxR > (row+parseInt(cell.rowSpan)-1) ? maxR : (row+parseInt(cell.rowSpan)-1);
	            	  maxC = maxC > (col+parseInt(cell.colSpan)-1) ? maxC : (col+parseInt(cell.colSpan)-1); 
	            	  
	            	  for(var i=maxR; i >= minR; i--){
	            		  for(var j= minC; j <= maxC; j++){
	            			  this.instance.wtTable.currentCellCache.add(i, j, this.settings.className);
	            			  
	            			  if(modifyCorners[0] > i){
	            				  modifyCorners[0] = i;
	            			  }else if(modifyCorners[2] < i){
	            				  modifyCorners[2] = i;
	            			  }
	            			  
	            			  if(modifyCorners[1] > j){
	            				  modifyCorners[1] = j;
	            			  }else if(modifyCorners[3] < j){
	            				  modifyCorners[3] = j;
	            			  }
	            		  }
	            	  }
	            	  
	            	 /* for(var i=row; i <= corners[2]; i++){
	            		  for(var j=col; j < col+cell.colSpan; j++){
	            			  this.instance.wtTable.currentCellCache.add(i, j, this.settings.className);
	            	  ...
	            	  for(var i=col; i >= corners[1]; i--){
	            		  for(var j=row; j < row+cell.rowSpan; j++){
	            			  this.instance.wtTable.currentCellCache.add(j, i, this.settings.className);
	            			  
	            			  if(modifyCorners[0] > j){
	            				  modifyCorners[0] = j;
	            			  }else if(modifyCorners[2] < j){
	            				  modifyCorners[2] = j;
	            			  }
	            			  
	            			  if(modifyCorners[1] > i){
	            				  modifyCorners[1] = i;
	            			  }else if(modifyCorners[3] < i){
	            				  modifyCorners[3] = i;
	            			  }
	            		  }
	            	  }
	            	  
	            	  for(var i=col; i <= corners[3]; i++){
	            		  for(var j=row; j < row+cell.rowSpan; j++){
	            			  this.instance.wtTable.currentCellCache.add(j, i, this.settings.className);
        	  ...
        	  }else if(this.settings.className == 'current'){
        		  if(cell.style.display == 'none'){
        			  var mergeCell = $(cell).data('mergeCell');
                	  modifyCorners[0] = mergeCell.row; 
                	  modifyCorners[1] = mergeCell.col; 
                	  modifyCorners[2] = mergeCell.row;
                	  modifyCorners[3] = mergeCell.col;
        		  }
        	  }
          }
          // jung 추가 end
          
          if(this.isLineAll && this.settings.className == 'area'){
        	  if(!(cell.colSpan > 1 || cell.rowSpan > 1 || cell.style.display == 'none')){
        		//selected cell          
    ...
    corners = modifyCorners;	// jung 추가, 셀병합 시 선택 영역 정상 동작 하도록 하기 위해 추가

    if(this.isLineAll && this.settings.className == 'area'){
    	this.border && this.border.appear2(corners); //warning! border.appear modifies corners!
    }else{
...
function WalkontableSettings(instance, settings) {
  ...
  this.defaults = {
    ...
    scrollbarModelH: 'native', //values: dragdealer, native
...
WalkontableTable.prototype.adjustColumns = function (TR, desiredCount) {
  ...
  while (count < desiredCount) {
    ...
    TR.appendChild(TD);
    //cellSettings에 스타일 정보를 담는 방식으로 변경 하여 이부분을 주석 처리 함
    //TD.style.height = (this.wtDom.outerHeight(TD)-2) + 'px';	//jung 추가, inline으로 height 값을 가지고 있도록 하기 위해 추가
    count++;
  }
  while (count > desiredCount) {
    TR.removeChild(TR.lastChild);
...
WalkontableTable.prototype._doDraw = function () {
  ...
  if (totalColumns > 0) {
    ...
    while (source_r < totalRows && source_r >= 0) {
      ...
      //jgs135 추가 행 th에 높이값 추가하는 로직
      var th = TR.firstChild;
      var height = this.instance.coreInstance.getRowHeight(this.getCoords(th)[0]);
...
//      if (r === 0) {
      ...
      for (c = 0; c < displayTds; c++) {
        ...
        TD.className = '';
        // jung 수정, 테이블 랜더링 시 테두리 style 삭제 되는 거 막음.
...
var // currently active contextMenu trigger
    ...
    handle = {
        ...
        contextmenu: function(e) {
            ...
            // jung 추가, 추가된 컨트롤에서 마우스 오른쪽 버튼을 클릭 할 경우 컨텍스 메뉴가 나오지 않도록 처리
            if(e.target.nodeName.toLowerCase() == 'button' || e.target.nodeName.toLowerCase() == 'img'){
            	return;
            }
            // jung 추가 end
        ...
        layerClick: function(e) {
            ...
            setTimeout(function() {
                ...
                if(root.$menu.css('display') != 'none' && triggerAction){
    ...
    op = {
        ...
        update: function(opt, root) {
            ...
            opt.$menu.children().each(function(){
                ...
                // jung 추가, 반복 취소 메뉴는 반복이 설정된 row에서만 표시되도록 추가
                if(key == 'remove_repeat'){
                	if(!$trigger.hasClass('th_repeat')){
                		$item.hide();
                	}else{
                		$item.show();
                	}
                }else if(key == 'repeat_set'){
                	if(!$trigger.hasClass('th_repeat')){
                		$item.show();
                	}else{
                		$item.hide();
                	}
                }else if(key == 'repeat_groupId'){
                	if(!$trigger.hasClass('th_repeat')){
                		$item.hide();
                	}else{
                		$item.show();
