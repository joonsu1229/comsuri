...
(function($){		
	$.fn.jqte = function(options){
		...
		var varsTitle = [
			{title:"Text Family"},	// jung 수정. format을 family로 기능 변경
			{title:"Font Size"},
			{title:"Color"},
			{title:"Bold",hotkey:"B"},
		...
		var formats = [["","기본 폰트"],["굴림, Gulim","굴림 (가나다라)"],["바탕, Batang","바탕 (가나다라)"],["돋움, Dotum","돋움 (가나다라)"],
			               ["궁서, Gungsuh","궁서 (가나다라)"],["Arial","Arial (abcdef)"],["Verdana","Verdana (abcdef)"]];
		// default font sizes
		...
		return this.each(function(){
			...
			jQTE.html('<div class="'+vars.css+"_toolbar"+'" role="toolbar" unselectable></div><div class="'+vars.css+'_linkform" style="display:none" role="dialog"></div><div class="'+vars.css+"_editor"+'" style="display:none;"></div>');
				...
				if(vars[buttons[n].name])
				{
					...
					if(buttons[n].name=="format" && $.isArray(vars.formats))
					{
						...
						for(var f = 0; f < vars.formats.length; f++)
						{
							...
							toolbar.find("."+vars.css+"_formats").append('<a '+vars.css+'-formatval="'+ vars.formats[f][0] +'" class="'+vars.css+'_format'+'" role="menuitem" unselectable style="font-family:'+vars.formats[f][0]+'">'+ vars.formats[f][1] +'</a>');
						}
			...
			function buttonEmphasize(e)
			{
				...
				if(vars.format && $.isArray(vars.formats))
				{
					...
					for(var f = 0; f < vars.formats.length; f++)
					{
						...
						*/
						var thisFormat = vars.formats[f][0];
						
						if(vars.formats[f][0].length>0 && detectFontFamily(thisFormat))
						{
							formatLabelView(vars.formats[f][1]);							
							isFoundFormat = true;
							break;
			...
			toolbutton
				...
				.click(function(e){	
					...
					{
						//jung 추가
						if(editor.css('opacity') == '0' || editor.css('display') == 'none'){
							if('fSize|colors|formats'.indexOf($(this).data('command')) == -1){
								return false;
							}
						}else{
							if('justifyLeft|justifyCenter|justifyRight'.indexOf($(this).data('command')) > -1){
								return false;
							}
						}
						// jung 추가 end
