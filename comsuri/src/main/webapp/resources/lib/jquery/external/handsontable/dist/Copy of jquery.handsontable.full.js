...
Handsontable.Core = function (rootElement, userSettings) {
  ...
  this.selection = selection = { //this public assignment is only temporary
    ...
    transformStart: function (rowDelta, colDelta, force) {
      ...
      var newSelectCell = instance.getCell(coords.row, coords.col);
      if(newSelectCell && newSelectCell.style.display == 'none'){
    	  
    	  var parentCoords = $(newSelectCell).data('mergeCell');
    	  
    	  if((coords.row - priv.selStart.row()) > 0){
    		  coords.row = parentCoords.row + 1;      		  
    	  }else if((coords.row - priv.selStart.row()) < 0){
    		  coords.row = parentCoords.row - 1;
    	  }else if((coords.col - priv.selStart.col()) > 0){
    		  coords.col = parentCoords.col + 1;      		  
    	  }else if((coords.col - priv.selStart.col()) < 0){
    		  coords.col = parentCoords.col - 1;
    	  }
      }
...
*/                break;
  ...
  this.cellMerge = function () {
	  ...
	  if(selected){
		  var firstCell = this.getCell(selected[0],selected[1]);
		  ...
		  for(var i = selected[2]; i >= selected[0]; i--){
			  for(var j = selected[3]; j >= selected[1]; j--){
				  ...
				  $(cell).hide();
				  $(cell).data('mergeCell',{row : selected[0], col : selected[1]});
			  }
		  }
		  
		  if(selected[2] - selected[0] > 0){
			  firstCell.rowSpan = selected[2] - selected[0] + 1; 
		  }
		  
		  if(selected[3] - selected[1] > 0){
			  firstCell.colSpan = selected[3] - selected[1] + 1;
		  }
		  
		  this.deselectCell();
		  this.selectCell(selected[0],selected[1],selected[0],selected[1]);
	  }
  };
  ...
  this.setCellAttribute = function(attrData){
	  ...
	  for (var r = 0; r < visibleRows; r++) {
		for (var c = 0; c < visibleColumns; c++) {
			...
			if(attr.rowSpan){
				cell.rowSpan = attr.rowSpan;
			}
			
			if(attr.style){
				$(cell).attr('style',attr.style);
			}
		}
	  }
  };
   ...
   * @author JUNG(jgs135@naonsoft.com)
   */
  this.cellOutLine = function(){
	  var selected = this.getCustomSelected();
	  
	  if(selected){
		  //left, right
		  for(var i = selected[0]; i <= selected[2]; i++){
			  var leftCell = this.getCell(i,selected[1]-1);
			  var rightCell = this.getCell(i, selected[3]+1);
			  
			  if(!rightCell || rightCell.style.display == 'none'){
				  rightCell = this.getCell(i, selected[3]);
				  rightCell.style.borderRight = '1px solid black';
			  }else{
				  rightCell.style.borderLeft = '1px solid black';
			  }
			  
			  if(leftCell.style.display == 'none'){
				  leftCell = this.getCell(i,selected[1]);
				  leftCell.style.borderLeft = '1px solid black';
			  }else{
				  leftCell.style.borderRight = '1px solid black';
			  }
		  }
		  //top, bottom
		  for(var i = selected[1]; i <= selected[3]; i++){
			  var topCell = this.getCell(selected[0]-1,i);
			  var bottomCell = this.getCell(selected[2]+1,i);
			  
			  if(!bottomCell || bottomCell.style.display == 'none'){
				  bottomCell = this.getCell(selected[2],i);
				  bottomCell.style.borderBottom = '1px solid black';
			  }else{
				  bottomCell.style.borderTop = '1px solid black';
			  }
			  
			  if(!topCell || topCell.style.display == 'none'){
				  topCell = this.getCell(selected[0],i);
				  topCell.style.borderTop = '1px solid black';
			  }else{
   ...
   * @author JUNG(jgs135@naonsoft.com)
   */
  this.cellAllOutLine = function(){
	  var selected = this.getCustomSelected();
	  var parentCoords,parentCell;
	  ...
	  if(selected){
		  ...
		  for(var i = selected[0]; i <= selected[2]; i++){
			  for(var j = selected[1]; j <= selected[3]; j++){
				  var cell = this.getCell(i,j);
				  var leftCell = j == 0 ? undefined : this.getCell(i,j-1);
				  var topCell = i == 0 ? undefined : this.getCell(i-1,j);
				  ...
				  }else if(leftCell.style.borderRight == ''){
					  cell.style.borderLeft = '1px solid black';
				  }
				  
				  if(!topCell){
					  cell.style.borderTop = '1px solid black';
				  }else if(topCell.style.borderBottom == ''){
					  cell.style.borderTop = '1px solid black';
				  }
				  
				  if(j == selected[3]){
					  var rightCell = this.getCell(i,j+1);
					  
					  if(rightCell){
						  rightCell.style.borderLeft = '';
					  }
					  
					  if(cell.style.display == 'none'){
						  parentCoords = $(cell).data('mergeCell');
						  parentCell = this.getCell(parentCoords.row, parentCoords.col);
						  parentCell.style.borderRight = '1px solid black';
					  }else{
				  ...
				  if(i == selected[2]){
					  var bottomCell = this.getCell(i+1,j);
					  
					  if(bottomCell){
						  bottomCell.style.borderTop = '';
					  }
					  
					  if(cell.style.display == 'none'){
						  parentCoords = $(cell).data('mergeCell');
						  parentCell = this.getCell(parentCoords.row, parentCoords.col);
						  parentCell.style.borderBottom = '1px solid black';
					  }else{
...
Handsontable.TableView.prototype.isTextSelectionAllowed = function (el) {
  if (el.nodeName === 'P' || $(el.parentNode).hasClass('handsontableInput')) {
    return (true);
  }
...
HandsontableTextEditorClass.prototype.createElements = function () {
  ...
  this.TEXTAREA = document.createElement('DIV');	//jung 수정, textarea를 div로 변경
  ...
  this.textareaStyle.padding = '0px 4px 0px 4px';//jung 추가
  this.textareaStyle.backgroundColor = '#fff';// jung 추가
  this.textareaStyle.imeMode = 'active';
  this.$textarea = $(this.TEXTAREA);
  //this.$textarea.popline();	//jung 추가, 에디터 기능 추가
...
HandsontableTextEditorClass.prototype.bindEvents = function () {
  var that = this;
  $('.popline').on('mousedown.popline', function(event){
	  event.stopImmediatePropagation();
  });
  ...
  $document.mousemove(function (e) {
    if (pressed) {
    	if(isCol){
    		...
    		newSize = setManualSize(currentRow, currentHeight); //save col width
    	...
    	if(isCol){
    		...
    		instance.view.wt.wtDom.removeClass(rowResizer, 'active');
	  	    ...
	  	    if(newSize != startHeight){
	  	    	var size = parseInt($(currentTH).css('height'))+(newSize-startHeight);
	  	    	$(currentTH.parentNode).children().each(function(){
	  	    		$(this).css('height',size);
	  	    	});
	  	    	//$(currentTH).css('height',parseInt($(currentTH).css('height'))+newSize); 
	  	    	/*instance.forceFullRender = true;
	  	    	instance.view.render(); //updates all
	  	    	saveManualColumnWidths.call(instance);
	  	    	instance.PluginHooks.run('afterColumnResize', currentCol, newSize);*/
...
CopyPasteClass.prototype.triggerPaste = function (event, str) {
  ...
  if (that.pasteCallbacks) {
    setTimeout(function () {
      var val = (str || that.elTEXTAREA.innerHTML).replace(/\n$/, ''); //remove trailing newline
