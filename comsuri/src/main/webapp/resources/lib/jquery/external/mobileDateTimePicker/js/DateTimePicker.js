...
;(function ( $, window, document, undefined ) {
		...
		var formatHumanDate = function(date) {
			return date.yyyy + "년 " + date.month + " " + date.dd + "일 " + date.day;
		};
		...
		var defaults = {
			...
			titleContentDate: "날짜 선택",
			titleContentTime: "시간 선택",
			titleContentDateTime: "날짜와 시간 선택",
		
			buttonsToDisplay: ["닫기", "설정", "지움"],
			setButtonContent: "설정",
			clearButtonContent: "지움",
		...
		DateTimePicker.prototype = {
			...
			_modifyPicker: function()
			{
				...
				{
					if(dtPickerObj._compare(dtPickerObj.settings.buttonsToDisplay[iTempIndex], "닫기"))
						bDisplayHeaderCloseButton = true;
					else if(dtPickerObj._compare(dtPickerObj.settings.buttonsToDisplay[iTempIndex], "설정"))
						bDisplaySetButton = true;
					else if(dtPickerObj._compare(dtPickerObj.settings.buttonsToDisplay[iTempIndex], "지움"))
			...
			_addEventHandlersForPicker: function()
			{
				...
				$(".dtpicker-compValue").keyup(function()
				{
					...
					if($oTextField.parent().hasClass("hour") || $oTextField.parent().hasClass("minutes") || $oTextField.parent().hasClass("meridiem"))
					{
						if(iLength > 2)
						{
					...
					else if($oTextField.parent().hasClass("day")){
						if(iLength > 2){
							var sNewTextBoxVal = sTextBoxVal.slice(0, 2);
							$oTextField.val(sNewTextBoxVal);
						}else{
							var monthChk = $('.dtpicker-header .dtpicker-value').text();
							var yearChk = monthChk.substring(0,4).replace(/[^0-9\.]/g,'');
							monthChk = monthChk.substring(6,9).replace(/[^0-9\.]/g,'');
							var lastDay = ( new Date( yearChk, monthChk, 0) ).getDate();	
							if(sTextBoxVal.slice(0, 2) > lastDay){
								$oTextField.val(lastDay);
							}
						}
					}
				...
				{
					//간격 설정 값만큼 늘어나도록 수정 jgs135
					//dtPickerObj.dataObject.iCurrentMinutes++;
					dtPickerObj.dataObject.iCurrentMinutes += dtPickerObj.settings.timeStep;
					dtPickerObj._setCurrentDate();
					dtPickerObj._setOutputOnIncrementOrDecrement();
				});
				...
				{
					//간격 설정 값만큼 줄어들도록 수정 jgs135
					//dtPickerObj.dataObject.iCurrentMinutes--;
					dtPickerObj.dataObject.iCurrentMinutes = dtPickerObj.dataObject.iCurrentMinutes - dtPickerObj.settings.timeStep;
					dtPickerObj._setCurrentDate();
					dtPickerObj._setOutputOnIncrementOrDecrement();
				});
			...
			_parseTime: function(sTime)
			{
				...
				if(sTime != "" && sTime != undefined && sTime != null)
				{
					...
					if(iMinutes.toString().substring(0,1) == "5") {
						if(iHour.toString() == "23") {
							iHour = parseInt("00");
							iMinutes = parseInt("00");
						} else {
							iHour = iHour+1;
							iMinutes = parseInt("00");
						}
					} else {
						iMinutes = Math.ceil(iMinutes/10)*10;						
					}					
				}
			...
			_getValuesFromInputBoxes: function()
			{
				...
				{
					dtPickerObj.dataObject.iCurrentHour = Number($(dtPickerObj.element).find(".hour .dtpicker-compValue").val());
					dtPickerObj.dataObject.iCurrentMinutes = Number($(dtPickerObj.element).find(".minutes .dtpicker-compValue").val());
					
					//분 간격에 맞지 않은 값이 입력 되었을 경우 간격에 맞게 조정 jgs135
					if(dtPickerObj.dataObject.iCurrentMinutes%dtPickerObj.settings.timeStep > 0){
						var fixedMinutes = parseInt(dtPickerObj.dataObject.iCurrentMinutes/dtPickerObj.settings.timeStep)*dtPickerObj.settings.timeStep;
						dtPickerObj.dataObject.iCurrentMinutes = fixedMinutes;
						$(dtPickerObj.element).find(".minutes .dtpicker-compValue").val(fixedMinutes);
					}
